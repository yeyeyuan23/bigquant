...
当前 84 组换手实验的完整入口见 [回测总览](../README.md)。本页保留早期三种多空规则、纯多头和分组评价的详细口径。

最新：[十分组换手优化结果](../results/20260917_turnover_trials/README.md)。AutoDL 已完成 21 个持仓配置 × 4 种成本情景并通过审计。10% 持仓 / 30% 保留带的换手为 **0.870**、毛年化 **59.66%**、扣费年化 **35.46%**、净 Sharpe **1.991**、净累计 **71.63%**、最大回撤 **−12.45%**；2025 和 2026 年至 8 月均优于每日十分组基线。全部配置、分年、滑点与起点敏感性都已保留；这是回顾性比较，尚非独立策略选择样本外验证。方案见 [TURNOVER_TRIALS.md](TURNOVER_TRIALS.md)。

演示稿当前的每日十分组基线：[每日 Q10−Q1 的扣费累计收益与回撤](../results/20260917_decile_strategy_preview/README.md)。两端各 10%，多空各 100%，每日调仓；扣费后日均单边换手 1.505、年化 13.52%、Sharpe 0.784、累计 21.17%、最大回撤 -14.76%。中证 1000 仅作市场参照。该图已纳入正式演示稿第 14 页，作为“可交易性检验”，排在因子有效性之后；输入消融与结构增量顺延到第 15、16 页。以下旧规则结果保留为研究历史。
...
[`protocol.json`](../src/protocol.json) 在看到本次结果之前固定了三种规则及零费率、基准费率、每侧另加 2bp/5bp 的 12 组配置；没有继续搜索策略参数。策略本身来自此前已经看过这段时期的研究，本次是回顾性比较，不能称为独立的策略选择样本外验证。
...
/root/autodl-tmp/conda-envs/quant/bin/python -m unittest discover -s backtest/src -p test_raw_backtest.py -v
/root/autodl-tmp/conda-envs/quant/bin/python backtest/src/run_raw_scores.py \
