...
- `candidate454_joint_full_history/`：基础 10 路 route batch 与评分审计。
- `frozen_full_history_replay_2019_2024/`：X/M/T 的六年冻结权重纯推理结果。
- `m_raw_final_checkpoint/`、`m_v3_final/`：M checkpoint。
- `m_multiaxis/`：三轴 M 的代码对应 checkpoint、日志和审计；明确保留。
- `en454_baseline/`：EN454 与 residual-T 依赖。
- `unified_alpha_fusion_suite_full_experts_v1/`：X-tree checkpoint。
- `xt_retrain/`：X-MLP/T checkpoint。

## archive

`archive/legacy_factor_pool/` 保存旧 S/I/T 因子池报告和旧说明，
`archive/legacy_pre_factorwise/` 保存更早的准入流程结果。它们不属于当前
