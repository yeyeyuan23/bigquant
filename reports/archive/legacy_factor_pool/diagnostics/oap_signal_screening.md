...
机器可读清单见 `reports/archive/legacy_factor_pool/diagnostics/oap_signal_shortlist.csv`。
...
精确结果见统一输出 `reports/archive/legacy_factor_pool/routes/factor_pool_incremental.csv` 和
`reports/archive/legacy_factor_pool/latest/factor_pool_admission.csv`。当前 `PV-003` 仅通过 S，进入规则复合；
