...
- `J_2019_2024_18_ROUTES_FULL.csv`: the main route table. This is the only
  compact table needed for comparing routes. It contains the complete scoring
  fields plus five run-audit columns.
...
The canonical CSV contains these 28 columns, in order. `score` is the stable
display alias of `J`; the five fields after it are shared run-audit metadata.
...
b_nonzero_window_ratio, score, score_days, score_weight_windows,
reference_factor_count, joint_route_count, joint_common_rows
```
