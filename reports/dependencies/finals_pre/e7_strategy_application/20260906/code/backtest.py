...
def execute(
    ...
    for _ in range(100):
        ...
        notional = np.abs(desired - before).sum()
        new_nav = nav - cost * notional
...
def run(
    data: Inputs, strategy: str, cost_bps: float, phase: int = 0, capture_positions: bool = False
...
def summarize(daily: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "cost_bps": float(daily["cost_bps"].iloc[0]),
...
def main() -> None:
    ...
    for strategy in STRATEGIES:
        for cost in (0, 5, 10, 20):
            ...
            days.append(daily)
    for phase in (1, 2):
        daily, _ = run(data, "rebalance_3d", 10, phase)
    ...
    primary = base[base["cost_bps"] == 10].copy()
    gross = base[base["cost_bps"] == 0].set_index("strategy")["annualized_return"]
    primary["gross_annualized_return_0bp"] = primary["strategy"].map(gross)
    primary.to_csv(args.out / "strategy_primary_10bp.csv", index=False)
    ...
    audit = {
        ...
        "execution_assumptions": [
            "idealized fills at observed opening price",
