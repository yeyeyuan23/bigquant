r"""INT-007: Fangzheng report candidate, source FZ-048.

Source: 方正证券《勇攀高峰》, 14–15
Original formula: \(Orth(44\mid 风格+行业)\)
Frozen logic: 剥离传统风格后的高波动补偿残差
...
Deduplication: FZ-044 at abs median daily Spearman 0.824569;
...
def build_int_007_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_int_007_daily(daily_features)
    result = panel.merge(
        daily,
