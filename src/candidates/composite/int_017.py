r"""INT-017: Fangzheng report candidate, source FZ-109.

Source: 方正证券《草木皆兵》, 9
Original formula: \(EW(107,108)\)
Frozen logic: 合成个人投资者维度
Data families: HF+PV
...
Deduplication: FZ-108 at abs median daily Spearman 0.712069;
...
def build_int_017_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_int_017_daily(daily_features)
    result = panel.merge(
        daily,
