r"""INT-006: Fangzheng report candidate, source FZ-029.

Source: 方正证券《适度冒险》, 12–13
Original formula: \(Orth(28\mid 10个风格因子)\)
Frozen logic: 剥离波动、换手等传统暴露
...
Deduplication: FZ-096 at abs median daily Spearman 0.763061;
...
def build_int_006_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_int_006_daily(daily_features)
    result = panel.merge(
        daily,
