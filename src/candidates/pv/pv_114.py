r"""PV-114: report-derived candidate, source GTJA-0085.

Source research ID: GTJA-0085
Frozen formula: (TSRANK((VOLUME / MEAN(VOLUME,20)), 20) * TSRANK((-1 * DELTA(CLOSE, 7)), 8))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0085"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0085"
ORIENTATION = 1.0
...
def build_pv_114_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_114_daily(daily_features)
    result = panel.merge(
        daily,
