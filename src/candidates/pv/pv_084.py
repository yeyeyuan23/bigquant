r"""PV-084: report-derived candidate, source GTJA-0047.

Source research ID: GTJA-0047
Frozen formula: SMA((TSMAX(HIGH,6)-CLOSE)/(TSMAX(HIGH,6)-TSMIN(LOW,6))*100,9,1)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0047"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0047"
ORIENTATION = 1.0
...
def build_pv_084_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_084_daily(daily_features)
    result = panel.merge(
        daily,
