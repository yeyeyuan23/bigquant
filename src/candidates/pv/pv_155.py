r"""PV-155: report-derived candidate, source GTJA-0128.

Source research ID: GTJA-0128
Frozen formula: 100-(100/(1+SUM(((HIGH+LOW+CLOSE)/3>DELAY((HIGH+LOW+CLOSE)/3,1)?(HIGH+LOW+CLOSE)/3*VOLUME:0),14)/SUM(((HIGH+LOW+CLOSE)/3<DELAY((HIGH+LOW+CLOSE)/3,1)?(HIGH+LOW+CLOSE)/3*VOLUME:0), 14)))
Economic logic: 成交活跃/资金状态：刻画量能异常、趋势、集中或价量交互
Data families: PV
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0128"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0128"
ORIENTATION = 1.0
...
def build_pv_155_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_155_daily(daily_features)
    result = panel.merge(
        daily,
