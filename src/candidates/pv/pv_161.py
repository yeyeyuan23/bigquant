r"""PV-161: report-derived candidate, source GTJA-0134.

Source research ID: GTJA-0134
Frozen formula: (CLOSE-DELAY(CLOSE,12))/DELAY(CLOSE,12)*VOLUME
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0134"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0134"
ORIENTATION = 1.0
...
def build_pv_161_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_161_daily(daily_features)
    result = panel.merge(
        daily,
