r"""PV-052: report-derived candidate, source GTJA-0008.

Source research ID: GTJA-0008
Frozen formula: RANK(DELTA(((((HIGH + LOW) / 2) * 0.2) + (VWAP * 0.8)), 4) * -1)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0008"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0008"
ORIENTATION = 1.0
...
def build_pv_052_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_052_daily(daily_features)
    result = panel.merge(
        daily,
