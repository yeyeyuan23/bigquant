r"""PV-030: Fangzheng report candidate, source FZ-055.

Source: 方正证券《球队硬币》, 14–15
Original formula: 每日按 \(\Delta TO_d\) 相对截面均值翻转 \(R^{oc}_d\)，再取均值
Frozen logic: 用换手变化修正日内反转
Data families: PV
...
Deduplication: FZ-051 at abs median daily Spearman 0.914319;
...
DATA_FAMILIES = ('PV',)
SOURCE_RESEARCH_ID = "FZ-055"
SOURCE_FIDELITY = "adapted_proxy"
COMPONENT_COLUMN = "FZ-055"
ORIENTATION = -1.0
...
def build_pv_030_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_030_daily(daily_features)
    result = panel.merge(
        daily,
