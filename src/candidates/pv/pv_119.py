r"""PV-119: report-derived candidate, source GTJA-0090.

Source research ID: GTJA-0090
Frozen formula: ( RANK(CORR(RANK(VWAP), RANK(VOLUME), 5)) * -1)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0090"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0090"
ORIENTATION = 1.0
...
def build_pv_119_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_119_daily(daily_features)
    result = panel.merge(
        daily,
