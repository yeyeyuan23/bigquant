r"""PV-055: report-derived candidate, source GTJA-0012.

Source research ID: GTJA-0012
Frozen formula: (RANK((OPEN - (SUM(VWAP , 10) / 10)))) * (-1 * (RANK(ABS((CLOSE - VWAP)))))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0012"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0012"
ORIENTATION = 1.0
...
def build_pv_055_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_055_daily(daily_features)
    result = panel.merge(
        daily,
