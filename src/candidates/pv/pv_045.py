r"""PV-045: report-derived candidate, source GTJA-0001.

Source research ID: GTJA-0001
Frozen formula: (-1 * CORR(RANK(DELTA(LOG(VOLUME), 1)), RANK(((CLOSE - OPEN) / OPEN)), 6))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0001"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0001"
ORIENTATION = 1.0
...
def build_pv_045_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_045_daily(daily_features)
    result = panel.merge(
        daily,
