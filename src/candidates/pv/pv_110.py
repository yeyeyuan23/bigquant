r"""PV-110: report-derived candidate, source GTJA-0080.

Source research ID: GTJA-0080
Frozen formula: (VOLUME-DELAY(VOLUME,5))/DELAY(VOLUME,5)*100
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0080"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0080"
ORIENTATION = 1.0
...
def build_pv_110_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_110_daily(daily_features)
    result = panel.merge(
        daily,
