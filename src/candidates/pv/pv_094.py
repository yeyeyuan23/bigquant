r"""PV-094: report-derived candidate, source GTJA-0060.

Source research ID: GTJA-0060
Frozen formula: SUM(((CLOSE-LOW)-(HIGH-CLOSE))./(HIGH-LOW).*VOLUME,20)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0060"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0060"
ORIENTATION = 1.0
...
def build_pv_094_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_094_daily(daily_features)
    result = panel.merge(
        daily,
