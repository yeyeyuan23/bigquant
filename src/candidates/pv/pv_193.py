r"""PV-193: report-derived candidate, source GTJA-0178.

Source research ID: GTJA-0178
Frozen formula: (CLOSE-DELAY(CLOSE,1))/DELAY(CLOSE,1)*VOLUME
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0178"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0178"
ORIENTATION = 1.0
...
def build_pv_193_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_193_daily(daily_features)
    result = panel.merge(
        daily,
