r"""PV-080: report-derived candidate, source GTJA-0042.

Source research ID: GTJA-0042
Frozen formula: ((-1 * RANK(STD(HIGH, 10))) * CORR(HIGH, VOLUME, 10))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0042"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0042"
ORIENTATION = 1.0
...
def build_pv_080_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_080_daily(daily_features)
    result = panel.merge(
        daily,
