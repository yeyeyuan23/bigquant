r"""PV-087: report-derived candidate, source GTJA-0052.

Source research ID: GTJA-0052
Frozen formula: SUM(MAX(0,HIGH-DELAY((HIGH+LOW+CLOSE)/3,1)),26)/SUM(MAX(0,DELAY((HIGH+LOW+CLOSE)/3,1)-LOW),26)*100
Economic logic: 日度价格形态：刻画典型价上方与下方的区间突破强度
Data families: PV
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0052"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0052"
ORIENTATION = 1.0
...
def build_pv_087_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_087_daily(daily_features)
    result = panel.merge(
        daily,
