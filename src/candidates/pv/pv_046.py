r"""PV-046: report-derived candidate, source GTJA-0002.

Source research ID: GTJA-0002
Frozen formula: (-1 * DELTA((((CLOSE - LOW) - (HIGH - CLOSE)) / (HIGH - LOW)), 1))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0002"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0002"
ORIENTATION = 1.0
...
def build_pv_046_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_046_daily(daily_features)
    result = panel.merge(
        daily,
