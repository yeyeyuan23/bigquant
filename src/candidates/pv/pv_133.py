r"""PV-133: report-derived candidate, source GTJA-0105.

Source research ID: GTJA-0105
Frozen formula: (-1 * CORR(RANK(OPEN), RANK(VOLUME), 10))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0105"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0105"
ORIENTATION = 1.0
...
def build_pv_133_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_133_daily(daily_features)
    result = panel.merge(
        daily,
