r"""PV-103: report-derived candidate, source GTJA-0071.

Source research ID: GTJA-0071
Frozen formula: (CLOSE-MEAN(CLOSE,24))/MEAN(CLOSE,24)*100
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0071"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0071"
ORIENTATION = 1.0
...
def build_pv_103_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_103_daily(daily_features)
    result = panel.merge(
        daily,
