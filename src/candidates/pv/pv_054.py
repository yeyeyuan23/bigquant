r"""PV-054: report-derived candidate, source GTJA-0011.

Source research ID: GTJA-0011
Frozen formula: SUM(((CLOSE-LOW)-(HIGH-CLOSE))./(HIGH-LOW).*VOLUME,6)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0011"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0011"
ORIENTATION = 1.0
...
def build_pv_054_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_054_daily(daily_features)
    result = panel.merge(
        daily,
