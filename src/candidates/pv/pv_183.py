r"""PV-183: report-derived candidate, source GTJA-0163.

Source research ID: GTJA-0163
Frozen formula: RANK(((((-1 * RET) * MEAN(VOLUME,20)) * VWAP) * (HIGH - CLOSE)))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0163"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0163"
ORIENTATION = 1.0
...
def build_pv_183_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_183_daily(daily_features)
    result = panel.merge(
        daily,
