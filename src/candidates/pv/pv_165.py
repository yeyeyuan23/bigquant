r"""PV-165: report-derived candidate, source GTJA-0139.

Source research ID: GTJA-0139
Frozen formula: (-1 * CORR(OPEN, VOLUME, 10))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0139"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0139"
ORIENTATION = 1.0
...
def build_pv_165_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_165_daily(daily_features)
    result = panel.merge(
        daily,
