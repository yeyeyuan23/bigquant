r"""PV-095: report-derived candidate, source GTJA-0061.

Source research ID: GTJA-0061
Frozen formula: (MAX(RANK(DECAYLINEAR(DELTA(VWAP , 1), 12)), RANK(DECAYLINEAR(RANK(CORR((LOW),MEAN(VOLUME,80), 8)), 17))) * -1)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0061"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0061"
ORIENTATION = 1.0
...
def build_pv_095_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_095_daily(daily_features)
    result = panel.merge(
        daily,
