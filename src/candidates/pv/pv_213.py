r"""PV-213: report-derived candidate, source HAITONG-0103.

Source research ID: HAITONG-0103
Frozen formula: max_84d(close)/min_84d(close)-1
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "HAITONG-0103"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "HAITONG-0103"
ORIENTATION = 1.0
...
def build_pv_213_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_213_daily(daily_features)
    result = panel.merge(
        daily,
