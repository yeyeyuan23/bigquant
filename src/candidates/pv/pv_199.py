r"""PV-199: report-derived candidate, source GTJA-0189.

Source research ID: GTJA-0189
Frozen formula: MEAN(ABS(CLOSE-MEAN(CLOSE,6)),6)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0189"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0189"
ORIENTATION = 1.0
...
def build_pv_199_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_199_daily(daily_features)
    result = panel.merge(
        daily,
