r"""PV-169: report-derived candidate, source GTJA-0145.

Source research ID: GTJA-0145
Frozen formula: (MEAN(VOLUME,9)-MEAN(VOLUME,26))/MEAN(VOLUME,12)*100
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0145"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0145"
ORIENTATION = 1.0
...
def build_pv_169_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_169_daily(daily_features)
    result = panel.merge(
        daily,
