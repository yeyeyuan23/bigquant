r"""PV-120: report-derived candidate, source GTJA-0091.

Source research ID: GTJA-0091
Frozen formula: ((RANK((CLOSE - MAX(CLOSE, 5)))*RANK(CORR((MEAN(VOLUME,40)), LOW, 5))) * -1)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0091"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0091"
ORIENTATION = 1.0
...
def build_pv_120_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_120_daily(daily_features)
    result = panel.merge(
        daily,
