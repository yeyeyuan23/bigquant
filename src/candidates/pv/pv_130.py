r"""PV-130: report-derived candidate, source GTJA-0102.

Source research ID: GTJA-0102
Frozen formula: SMA(MAX(VOLUME-DELAY(VOLUME,1),0),6,1)/SMA(ABS(VOLUME-DELAY(VOLUME,1)),6,1)*100
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0102"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0102"
ORIENTATION = 1.0
...
def build_pv_130_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_130_daily(daily_features)
    result = panel.merge(
        daily,
