r"""PV-109: report-derived candidate, source GTJA-0079.

Source research ID: GTJA-0079
Frozen formula: SMA(MAX(CLOSE-DELAY(CLOSE,1),0),12,1)/SMA(ABS(CLOSE-DELAY(CLOSE,1)),12,1)*100
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0079"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0079"
ORIENTATION = 1.0
...
def build_pv_109_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_109_daily(daily_features)
    result = panel.merge(
        daily,
