r"""PV-091: report-derived candidate, source GTJA-0057.

Source research ID: GTJA-0057
Frozen formula: SMA((CLOSE-TSMIN(LOW,9))/(TSMAX(HIGH,9)-TSMIN(LOW,9))*100,3,1)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0057"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0057"
ORIENTATION = 1.0
...
def build_pv_091_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_091_daily(daily_features)
    result = panel.merge(
        daily,
