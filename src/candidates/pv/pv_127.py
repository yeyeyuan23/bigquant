r"""PV-127: report-derived candidate, source GTJA-0099.

Source research ID: GTJA-0099
Frozen formula: (-1 * RANK(COVIANCE(RANK(CLOSE), RANK(VOLUME), 5)))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0099"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0099"
ORIENTATION = 1.0
...
def build_pv_127_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_127_daily(daily_features)
    result = panel.merge(
        daily,
