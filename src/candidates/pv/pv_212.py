r"""PV-212: report-derived candidate, source HAITONG-0102.

Source research ID: HAITONG-0102
Frozen formula: max_63d(close)/min_63d(close)-1
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "HAITONG-0102"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "HAITONG-0102"
ORIENTATION = 1.0
...
def build_pv_212_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_212_daily(daily_features)
    result = panel.merge(
        daily,
