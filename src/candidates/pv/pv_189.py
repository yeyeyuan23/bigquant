r"""PV-189: report-derived candidate, source GTJA-0171.

Source research ID: GTJA-0171
Frozen formula: ((-1 * ((LOW - CLOSE) * (OPEN^5))) / ((CLOSE - HIGH) * (CLOSE^5)))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0171"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0171"
ORIENTATION = 1.0
...
def build_pv_189_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_189_daily(daily_features)
    result = panel.merge(
        daily,
