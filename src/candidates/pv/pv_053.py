r"""PV-053: report-derived candidate, source GTJA-0009.

Source research ID: GTJA-0009
Frozen formula: SMA(((HIGH+LOW)/2-(DELAY(HIGH,1)+DELAY(LOW,1))/2)*(HIGH-LOW)/VOLUME,7,2)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0009"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0009"
ORIENTATION = 1.0
...
def build_pv_053_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_053_daily(daily_features)
    result = panel.merge(
        daily,
