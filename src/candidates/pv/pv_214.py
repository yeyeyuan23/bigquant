r"""PV-214: report-derived candidate, source HAITONG-0104.

Source research ID: HAITONG-0104
Frozen formula: max_105d(close)/min_105d(close)-1
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "HAITONG-0104"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "HAITONG-0104"
ORIENTATION = 1.0
...
def build_pv_214_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_214_daily(daily_features)
    result = panel.merge(
        daily,
