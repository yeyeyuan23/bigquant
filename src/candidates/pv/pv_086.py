r"""PV-086: report-derived candidate, source GTJA-0049.

Source research ID: GTJA-0049
Frozen formula: SUM(((HIGH+LOW)>=(DELAY(HIGH,1)+DELAY(LOW,1))?0:MAX(ABS(HIGH-DELAY(HIGH,1)),ABS(LOW-DELAY(LOW,1)))),12)/(SUM(((HIGH+LOW)>=(DELAY(HIGH,1)+DELAY(LOW,1))?0:MAX(ABS(HIGH-DELAY(HIGH,1)),ABS(LOW-DELAY(LOW,1)))),12)+SUM(((HIGH+LOW)<=(DELAY(HIGH,1)+DELAY(LOW,1))?0:MAX(ABS(HIGH-DELAY(HIGH,1)),ABS(LOW-DELAY(LOW,1)))),12))
Economic logic: 日度价格形态：刻画K线位置、缺口、均价偏离或区间突破/反转
Data families: PV
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0049"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0049"
ORIENTATION = 1.0
...
def build_pv_086_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_086_daily(daily_features)
    result = panel.merge(
        daily,
