r"""PV-025: Fangzheng report candidate, source FZ-050.

Source: 方正证券《球队硬币》, 7–9
Original formula: 若 \(\sigma_{20}(R^{cc}_i)<M(\sigma_{20})\)，取
  \(-\mu_{20}(R^{cc})\)，否则不变
Frozen logic: 以波动率识别动量股并翻转
Data families: PV
...
Deduplication: FZ-054 at abs median daily Spearman 0.712380;
...
DATA_FAMILIES = ('PV',)
SOURCE_RESEARCH_ID = "FZ-050"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "FZ-050"
ORIENTATION = -1.0
...
def build_pv_025_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_025_daily(daily_features)
    result = panel.merge(
        daily,
