r"""PV-132: report-derived candidate, source GTJA-0104.

Source research ID: GTJA-0104
Frozen formula: (-1 * (DELTA(CORR(HIGH, VOLUME, 5), 5) * RANK(STD(CLOSE, 20))))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0104"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0104"
ORIENTATION = 1.0
...
def build_pv_132_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_132_daily(daily_features)
    result = panel.merge(
        daily,
