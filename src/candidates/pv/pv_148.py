r"""PV-148: report-derived candidate, source GTJA-0120.

Source research ID: GTJA-0120
Frozen formula: (RANK((VWAP - CLOSE)) / RANK((VWAP + CLOSE)))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0120"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0120"
ORIENTATION = 1.0
...
def build_pv_148_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_148_daily(daily_features)
    result = panel.merge(
        daily,
