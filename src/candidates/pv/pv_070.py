r"""PV-070: report-derived candidate, source GTJA-0029.

Source research ID: GTJA-0029
Frozen formula: (CLOSE-DELAY(CLOSE,6))/DELAY(CLOSE,6)*VOLUME
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0029"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0029"
ORIENTATION = 1.0
...
def build_pv_070_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_070_daily(daily_features)
    result = panel.merge(
        daily,
