r"""PV-149: report-derived candidate, source GTJA-0121.

Source research ID: GTJA-0121
Frozen formula: ((RANK((VWAP - MIN(VWAP , 12)))^TSRANK(CORR(TSRANK(VWAP , 20), TSRANK(MEAN(VOLUME,60), 2), 18), 3)) * -1)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0121"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0121"
ORIENTATION = 1.0
...
def build_pv_149_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_149_daily(daily_features)
    result = panel.merge(
        daily,
