r"""PV-092: report-derived candidate, source GTJA-0058.

Source research ID: GTJA-0058
Frozen formula: COUNT(CLOSE>DELAY(CLOSE,1),20)/20*100
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0058"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0058"
ORIENTATION = 1.0
...
def build_pv_092_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_092_daily(daily_features)
    result = panel.merge(
        daily,
