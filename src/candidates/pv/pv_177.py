r"""PV-177: report-derived candidate, source GTJA-0156.

Source research ID: GTJA-0156
Frozen formula: (MAX(RANK(DECAYLINEAR(DELTA(VWAP , 5), 3)), RANK(DECAYLINEAR(((DELTA(((OPEN * 0.15) + (LOW *0.85)), 2) / ((OPEN * 0.15) + (LOW * 0.85))) * -1), 3))) * -1)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0156"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0156"
ORIENTATION = 1.0
...
def build_pv_177_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_177_daily(daily_features)
    result = panel.merge(
        daily,
