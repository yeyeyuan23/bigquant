r"""PV-113: report-derived candidate, source GTJA-0084.

Source research ID: GTJA-0084
Frozen formula: SUM((CLOSE>DELAY(CLOSE,1)?VOLUME:(CLOSE<DELAY(CLOSE,1)?-VOLUME:0)),20)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0084"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0084"
ORIENTATION = 1.0
...
def build_pv_113_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_113_daily(daily_features)
    result = panel.merge(
        daily,
