r"""PV-115: report-derived candidate, source GTJA-0086.

Source research ID: GTJA-0086
Frozen formula: ((0.25 < (((DELAY(CLOSE, 20) - DELAY(CLOSE, 10)) / 10) - ((DELAY(CLOSE, 10) - CLOSE) / 10))) ? ( -1 * 1) : (((((DELAY(CLOSE, 20) - DELAY(CLOSE, 10)) / 10) - ((DELAY(CLOSE, 10) - CLOSE) / 10)) < 0) ? 1 : (( -1 * 1) * (CLOSE - DELAY(CLOSE, 1)))))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0086"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0086"
ORIENTATION = 1.0
...
def build_pv_115_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_115_daily(daily_features)
    result = panel.merge(
        daily,
