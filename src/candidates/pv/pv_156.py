r"""PV-156: report-derived candidate, source GTJA-0129.

Source research ID: GTJA-0129
Frozen formula: SUM((CLOSE-DELAY(CLOSE,1)<0?ABS(CLOSE-DELAY(CLOSE,1)):0),12)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0129"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0129"
ORIENTATION = 1.0
...
def build_pv_156_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_156_daily(daily_features)
    result = panel.merge(
        daily,
