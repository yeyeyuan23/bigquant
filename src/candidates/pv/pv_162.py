r"""PV-162: report-derived candidate, source GTJA-0135.

Source research ID: GTJA-0135
Frozen formula: SMA(DELAY(CLOSE/DELAY(CLOSE,20),1),20,1)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0135"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0135"
ORIENTATION = 1.0
...
def build_pv_162_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_162_daily(daily_features)
    result = panel.merge(
        daily,
