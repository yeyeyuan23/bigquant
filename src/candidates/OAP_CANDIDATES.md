...
- 预热期无值不算技术失败，但没有开发期有效截面时，S/I 明确失败。
- 严格按 `S → I → T` 缩小候选池：S 通过后进入规则复合并成为 I 候选，I 通过后
  进入 Elastic Net 并成为 T 候选，T 通过后进入 LightGBM。
- T 只评价 frozen I，不能绕过 S 或 I；T 的正交门槛独立于最终 Elastic Net 权重，
  但不独立于漏斗成员关系。
