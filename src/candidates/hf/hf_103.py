r"""HF-103: report-derived candidate, source HAITONG-0132.

Source research ID: HAITONG-0132
Frozen formula: mean_20d(tail_60_amount/full_day_amount)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "HAITONG-0132"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "HAITONG-0132"
ORIENTATION = 1.0
...
def build_hf_103_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_103_daily(daily_features)
    result = panel.merge(
        daily,
