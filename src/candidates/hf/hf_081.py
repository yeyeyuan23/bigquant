r"""HF-081: report-derived candidate, source CICC-013.

Source research ID: CICC-013
Frozen formula: sum(log_return of 20 highest-volume minutes; stable timestamp ties)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CICC-013"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CICC-013"
ORIENTATION = 1.0
...
def build_hf_081_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_081_daily(daily_features)
    result = panel.merge(
        daily,
