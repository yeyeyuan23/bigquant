r"""HF-122: report-derived candidate, source CJ-G015-V01.

Source research ID: CJ-G015-V01
Frozen formula: mean_21d(non-opening 5m return; equal weighting)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G015-V01"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G015-V01"
ORIENTATION = 1.0
...
def build_hf_122_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_122_daily(daily_features)
    result = panel.merge(
        daily,
