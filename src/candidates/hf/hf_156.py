r"""HF-156: report-derived candidate, source CJ-G042-V07.

Source research ID: CJ-G042-V07
Frozen formula: mean_20d(volume peaks above mean+2sigma, min gap=0m)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G042-V07"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G042-V07"
ORIENTATION = 1.0
...
def build_hf_156_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_156_daily(daily_features)
    result = panel.merge(
        daily,
