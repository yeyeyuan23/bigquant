r"""HF-158: report-derived candidate, source CJ-G042-V09.

Source research ID: CJ-G042-V09
Frozen formula: mean_20d(volume peaks above mean+2sigma, min gap=5m)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G042-V09"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G042-V09"
ORIENTATION = 1.0
...
def build_hf_158_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_158_daily(daily_features)
    result = panel.merge(
        daily,
