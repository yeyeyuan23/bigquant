r"""HF-109: report-derived candidate, source CJ-G007-V02.

Source research ID: CJ-G007-V02
Frozen formula: mean_21d(10m low15% volume-segment return)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G007-V02"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G007-V02"
ORIENTATION = 1.0
...
def build_hf_109_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_109_daily(daily_features)
    result = panel.merge(
        daily,
