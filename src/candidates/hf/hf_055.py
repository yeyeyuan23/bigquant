r"""HF-055: Fangzheng report candidate, source FZ-040.

Source: 方正证券《勇攀高峰》, 5
Original formula: \(\sigma_{20}(Cov_d)\)
Frozen logic: 反应不足状态的稳定程度
Data families: HF
...
Deduplication: FZ-041 at abs median daily Spearman 0.809460;
...
DATA_FAMILIES = ('HF',)
SOURCE_RESEARCH_ID = "FZ-040"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "FZ-040"
ORIENTATION = 1.0
...
def build_hf_055_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_055_daily(daily_features)
    result = panel.merge(
        daily,
