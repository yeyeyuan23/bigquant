r"""HF-056: Fangzheng report candidate, source FZ-041.

Source: 方正证券《勇攀高峰》, 5
Original formula: \(EW(39,40)\)
Frozen logic: 风险未获补偿后可能出现补涨修复
Data families: HF
...
Deduplication: FZ-039 at abs median daily Spearman 0.886719;
...
DATA_FAMILIES = ('HF',)
SOURCE_RESEARCH_ID = "FZ-041"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "FZ-041"
ORIENTATION = 1.0
...
def build_hf_056_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_056_daily(daily_features)
    result = panel.merge(
        daily,
