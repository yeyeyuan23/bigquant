r"""HF-126: report-derived candidate, source CJ-G020-V02.

Source research ID: CJ-G020-V02
Frozen formula: mean_21d(intraday std(30m return))
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G020-V02"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G020-V02"
ORIENTATION = 1.0
...
def build_hf_126_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_126_daily(daily_features)
    result = panel.merge(
        daily,
