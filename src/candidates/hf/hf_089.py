r"""HF-089: report-derived candidate, source CICC-076.

Source research ID: CICC-076
Frozen formula: sum(first20 log_return*minute_volume_share)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CICC-076"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CICC-076"
ORIENTATION = 1.0
...
def build_hf_089_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_089_daily(daily_features)
    result = panel.merge(
        daily,
