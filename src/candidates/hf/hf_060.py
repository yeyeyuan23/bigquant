r"""HF-060: Fangzheng report candidate, source FZ-045.

Source: 方正证券《勇攀高峰》, 12
Original formula: 将 \(OV_t\) 替换为收盘价滚动波动率后的月均协方差
Frozen logic: 不使用分钟内高低价的低信息版本
Data families: HF
...
Deduplication: FZ-047 at abs median daily Spearman 0.783904;
...
DATA_FAMILIES = ('HF',)
SOURCE_RESEARCH_ID = "FZ-045"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "FZ-045"
ORIENTATION = 1.0
...
def build_hf_060_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_060_daily(daily_features)
    result = panel.merge(
        daily,
