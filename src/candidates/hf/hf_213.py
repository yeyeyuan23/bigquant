r"""HF-213: report-derived candidate, source CJ-G103-V02.

Source research ID: CJ-G103-V02
Frozen formula: mean_21d(slope(log(absret)~log(amount)))
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G103-V02"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G103-V02"
ORIENTATION = 1.0
...
def build_hf_213_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_213_daily(daily_features)
    result = panel.merge(
        daily,
