r"""HF-083: report-derived candidate, source CICC-018.

Source research ID: CICC-018
Frozen formula: sqrt(sum(r^2 where r>0))
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CICC-018"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CICC-018"
ORIENTATION = 1.0
...
def build_hf_083_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_083_daily(daily_features)
    result = panel.merge(
        daily,
