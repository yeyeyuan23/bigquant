r"""HF-164: report-derived candidate, source CJ-G046-V01.

Source research ID: CJ-G046-V01
Frozen formula: mean_21d(5m pvol quintile 1 local statistic)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G046-V01"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G046-V01"
ORIENTATION = 1.0
...
def build_hf_164_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_164_daily(daily_features)
    result = panel.merge(
        daily,
