r"""HF-096: report-derived candidate, source HAITONG-0047.

Source research ID: HAITONG-0047
Frozen formula: mean_20d(m5_all_offsets_skew_central)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "HAITONG-0047"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "HAITONG-0047"
ORIENTATION = 1.0
...
def build_hf_096_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_096_daily(daily_features)
    result = panel.merge(
        daily,
