r"""HF-099: report-derived candidate, source HAITONG-0052.

Source research ID: HAITONG-0052
Frozen formula: mean_20d(m5_all_offsets_kurt_central)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "HAITONG-0052"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "HAITONG-0052"
ORIENTATION = 1.0
...
def build_hf_099_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_099_daily(daily_features)
    result = panel.merge(
        daily,
