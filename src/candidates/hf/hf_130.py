r"""HF-130: report-derived candidate, source CJ-G029-V04.

Source research ID: CJ-G029-V04
Frozen formula: amount-weighted active-share mapping version 4
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G029-V04"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G029-V04"
ORIENTATION = 1.0
...
def build_hf_130_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_130_daily(daily_features)
    result = panel.merge(
        daily,
