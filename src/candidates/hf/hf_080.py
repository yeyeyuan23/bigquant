r"""HF-080: report-derived candidate, source CICC-012.

Source research ID: CICC-012
Frozen formula: sum(log_return of 50 lowest-volume minutes; stable timestamp ties)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CICC-012"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CICC-012"
ORIENTATION = 1.0
...
def build_hf_080_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_080_daily(daily_features)
    result = panel.merge(
        daily,
