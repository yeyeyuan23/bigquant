r"""HF-113: report-derived candidate, source CJ-G011-V03.

Source research ID: CJ-G011-V03
Frozen formula: mean_21d(10m structured volume-tail return; q=5%)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G011-V03"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G011-V03"
ORIENTATION = 1.0
...
def build_hf_113_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_113_daily(daily_features)
    result = panel.merge(
        daily,
