r"""HF-038: Fangzheng report candidate, source FZ-005.

Source: 方正证券《多空博弈》, 6
Original formula:
  \(B_d(V,Pos)\)，\(Pos_t=\frac12[(C_t/L^{run}_t-1)+(C_t/H^{run}_t-1)]\)
Frozen logic: 用相对此前低点的涨幅和相对此前高点的跌幅判断多空主导
Data families: HF
...
Deduplication: FZ-001 at abs median daily Spearman 0.670326;
...
DATA_FAMILIES = ('HF',)
SOURCE_RESEARCH_ID = "FZ-005"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "FZ-005"
ORIENTATION = -1.0
...
def build_hf_038_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_038_daily(daily_features)
    result = panel.merge(
        daily,
