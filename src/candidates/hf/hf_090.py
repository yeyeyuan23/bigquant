r"""HF-090: report-derived candidate, source CICC-078.

Source research ID: CICC-078
Frozen formula: sum(first20 negative abs(log_return)*minute_volume_share)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CICC-078"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CICC-078"
ORIENTATION = 1.0
...
def build_hf_090_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_090_daily(daily_features)
    result = panel.merge(
        daily,
