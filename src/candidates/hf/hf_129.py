r"""HF-129: report-derived candidate, source CJ-G029-V01.

Source research ID: CJ-G029-V01
Frozen formula: amount-weighted active-share mapping version 1
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G029-V01"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G029-V01"
ORIENTATION = 1.0
...
def build_hf_129_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_129_daily(daily_features)
    result = panel.merge(
        daily,
