r"""HF-106: report-derived candidate, source CJ-G006-V02.

Source research ID: CJ-G006-V02
Frozen formula: mean_21d(sum(volume*ret)/sum(volume), 30m)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G006-V02"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G006-V02"
ORIENTATION = 1.0
...
def build_hf_106_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_106_daily(daily_features)
    result = panel.merge(
        daily,
