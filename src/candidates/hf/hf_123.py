r"""HF-123: report-derived candidate, source CJ-G015-V02.

Source research ID: CJ-G015-V02
Frozen formula: mean_21d(non-opening 5m return; volume weighting)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G015-V02"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G015-V02"
ORIENTATION = 1.0
...
def build_hf_123_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_123_daily(daily_features)
    result = panel.merge(
        daily,
