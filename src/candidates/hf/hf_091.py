r"""HF-091: report-derived candidate, source CICC-079.

Source research ID: CICC-079
Frozen formula: sum(first20 positive log_return*minute_volume_share)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CICC-079"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CICC-079"
ORIENTATION = 1.0
...
def build_hf_091_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_091_daily(daily_features)
    result = panel.merge(
        daily,
