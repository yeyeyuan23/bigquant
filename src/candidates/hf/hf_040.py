r"""HF-040: Fangzheng report candidate, source FZ-010.

Source: 方正证券《多空博弈》, 8
Original formula: \(B_d(Amp,r^{(5m)})\)，\(Amp_t=(H_t-L_t)/C_t\)
Frozen logic: 将成交量替换为一分钟振幅，刻画价格争夺幅度
Data families: HF
...
Deduplication: FZ-001 at abs median daily Spearman 0.663550;
...
DATA_FAMILIES = ('HF',)
SOURCE_RESEARCH_ID = "FZ-010"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "FZ-010"
ORIENTATION = -1.0
...
def build_hf_040_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_040_daily(daily_features)
    result = panel.merge(
        daily,
