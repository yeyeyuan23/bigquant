r"""PV-172: report-derived candidate, source GTJA-0150.

Source research ID: GTJA-0150
Frozen formula: (CLOSE+HIGH+LOW)/3*VOLUME
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0150"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0150"
ORIENTATION = 1.0
...
def build_pv_172_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_172_daily(daily_features)
    result = panel.merge(
        daily,
