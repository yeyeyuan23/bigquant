r"""PV-147: report-derived candidate, source GTJA-0119.

Source research ID: GTJA-0119
Frozen formula: (RANK(DECAYLINEAR(CORR(VWAP , SUM(MEAN(VOLUME,5), 26), 5), 7)) - RANK(DECAYLINEAR(TSRANK(MIN(CORR(RANK(OPEN), RANK(MEAN(VOLUME,15)), 21), 9), 7), 8)))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0119"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0119"
ORIENTATION = 1.0
...
def build_pv_147_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_147_daily(daily_features)
    result = panel.merge(
        daily,
