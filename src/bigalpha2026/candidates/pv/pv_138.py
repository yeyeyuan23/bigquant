r"""PV-138: report-derived candidate, source GTJA-0110.

Source research ID: GTJA-0110
Frozen formula: SUM(MAX(0,HIGH-DELAY(CLOSE,1)),20)/SUM(MAX(0,DELAY(CLOSE,1)-LOW),20)*100
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0110"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0110"
ORIENTATION = 1.0
...
def build_pv_138_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_138_daily(daily_features)
    result = panel.merge(
        daily,
