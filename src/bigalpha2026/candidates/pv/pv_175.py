r"""PV-175: report-derived candidate, source GTJA-0154.

Source research ID: GTJA-0154
Frozen formula: (((VWAP - MIN(VWAP , 16))) < (CORR(VWAP , MEAN(VOLUME,180), 18)))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0154"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0154"
ORIENTATION = 1.0
...
def build_pv_175_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_175_daily(daily_features)
    result = panel.merge(
        daily,
