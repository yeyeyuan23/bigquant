r"""PV-050: report-derived candidate, source GTJA-0006.

Source research ID: GTJA-0006
Frozen formula: (RANK(SIGN(DELTA((((OPEN * 0.85) + (HIGH * 0.15))), 4)))* -1)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0006"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0006"
ORIENTATION = 1.0
...
def build_pv_050_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_050_daily(daily_features)
    result = panel.merge(
        daily,
