r"""PV-215: report-derived candidate, source HAITONG-0105.

Source research ID: HAITONG-0105
Frozen formula: max_126d(close)/min_126d(close)-1
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "HAITONG-0105"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "HAITONG-0105"
ORIENTATION = 1.0
...
def build_pv_215_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_215_daily(daily_features)
    result = panel.merge(
        daily,
