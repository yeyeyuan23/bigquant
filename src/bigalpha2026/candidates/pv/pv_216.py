r"""PV-216: report-derived candidate, source HAITONG-0106.

Source research ID: HAITONG-0106
Frozen formula: max_252d(close)/min_252d(close)-1
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "HAITONG-0106"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "HAITONG-0106"
ORIENTATION = 1.0
...
def build_pv_216_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_216_daily(daily_features)
    result = panel.merge(
        daily,
