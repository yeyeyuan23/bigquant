r"""PV-158: report-derived candidate, source GTJA-0131.

Source research ID: GTJA-0131
Frozen formula: (RANK(DELAT(VWAP , 1))^TSRANK(CORR(CLOSE,MEAN(VOLUME,50), 18), 18))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0131"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0131"
ORIENTATION = 1.0
...
def build_pv_158_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_158_daily(daily_features)
    result = panel.merge(
        daily,
