r"""PV-039: Fangzheng report candidate, source FZ-092.

Source: 方正证券《飞蛾扑火》, 9
Original formula: 用从 \(L_{d-1}\) 到 \(H_d\) 的单/复利泰勒残项作日度跳跃分类，再翻转 \(Amp_d\)
Frozen logic: 仅用日频高低价识别向上跳跃
Data families: PV
...
Deduplication: FZ-090 at abs median daily Spearman 0.493315;
...
DATA_FAMILIES = ('PV',)
SOURCE_RESEARCH_ID = "FZ-092"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "FZ-092"
ORIENTATION = -1.0
...
def build_pv_039_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_039_daily(daily_features)
    result = panel.merge(
        daily,
