r"""PV-201: report-derived candidate, source GTJA-0191.

Source research ID: GTJA-0191
Frozen formula: ((CORR(MEAN(VOLUME,20), LOW, 5) + ((HIGH + LOW) / 2)) - CLOSE)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0191"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0191"
ORIENTATION = 1.0
...
def build_pv_201_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_201_daily(daily_features)
    result = panel.merge(
        daily,
