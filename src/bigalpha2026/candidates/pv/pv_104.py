r"""PV-104: report-derived candidate, source GTJA-0073.

Source research ID: GTJA-0073
Frozen formula: ((TSRANK(DECAYLINEAR(DECAYLINEAR(CORR((CLOSE), VOLUME, 10), 16), 4), 5) - RANK(DECAYLINEAR(CORR(VWAP , MEAN(VOLUME,30), 4),3))) * -1)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0073"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0073"
ORIENTATION = 1.0
...
def build_pv_104_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_104_daily(daily_features)
    result = panel.merge(
        daily,
