r"""PV-185: report-derived candidate, source GTJA-0167.

Source research ID: GTJA-0167
Frozen formula: SUM((CLOSE-DELAY(CLOSE,1)>0?CLOSE-DELAY(CLOSE,1):0),12)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0167"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0167"
ORIENTATION = 1.0
...
def build_pv_185_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_185_daily(daily_features)
    result = panel.merge(
        daily,
