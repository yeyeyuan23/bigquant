r"""PV-179: report-derived candidate, source GTJA-0158.

Source research ID: GTJA-0158
Frozen formula: ((HIGH-SMA(CLOSE,15,2))-(LOW-SMA(CLOSE,15,2)))/CLOSE
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0158"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0158"
ORIENTATION = 1.0
...
def build_pv_179_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_179_daily(daily_features)
    result = panel.merge(
        daily,
