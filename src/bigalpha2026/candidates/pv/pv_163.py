r"""PV-163: report-derived candidate, source GTJA-0136.

Source research ID: GTJA-0136
Frozen formula: ((-1 * RANK(DELTA(RET, 3))) * CORR(OPEN, VOLUME, 10))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0136"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0136"
ORIENTATION = 1.0
...
def build_pv_163_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_163_daily(daily_features)
    result = panel.merge(
        daily,
