r"""PV-081: report-derived candidate, source GTJA-0043.

Source research ID: GTJA-0043
Frozen formula: SUM((CLOSE>DELAY(CLOSE,1)?VOLUME:(CLOSE<DELAY(CLOSE,1)?-VOLUME:0)),6)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0043"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0043"
ORIENTATION = 1.0
...
def build_pv_081_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_081_daily(daily_features)
    result = panel.merge(
        daily,
