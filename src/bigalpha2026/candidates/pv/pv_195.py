r"""PV-195: report-derived candidate, source GTJA-0184.

Source research ID: GTJA-0184
Frozen formula: (RANK(CORR(DELAY((OPEN - CLOSE), 1), CLOSE, 200)) + RANK((OPEN - CLOSE)))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0184"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0184"
ORIENTATION = 1.0
...
def build_pv_195_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_195_daily(daily_features)
    result = panel.merge(
        daily,
