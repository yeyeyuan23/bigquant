r"""PV-037: Fangzheng report candidate, source FZ-062.

Source: 方正证券《球队硬币》, 20
Original formula: \(EW(52,56,61)\)
Frozen logic: 日间、日内、隔夜三个反转维度统一修正
Data families: PV
...
Deduplication: FZ-052 at abs median daily Spearman 0.870179;
...
DATA_FAMILIES = ('PV',)
SOURCE_RESEARCH_ID = "FZ-062"
SOURCE_FIDELITY = "adapted_proxy"
COMPONENT_COLUMN = "FZ-062"
ORIENTATION = -1.0
...
def build_pv_037_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_037_daily(daily_features)
    result = panel.merge(
        daily,
