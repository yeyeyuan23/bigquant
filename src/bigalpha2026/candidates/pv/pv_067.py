r"""PV-067: report-derived candidate, source GTJA-0026.

Source research ID: GTJA-0026
Frozen formula: ((((SUM(CLOSE, 7) / 7) - CLOSE)) + ((CORR(VWAP , DELAY(CLOSE, 5), 230))))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0026"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0026"
ORIENTATION = 1.0
...
def build_pv_067_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_067_daily(daily_features)
    result = panel.merge(
        daily,
