r"""PV-072: report-derived candidate, source GTJA-0032.

Source research ID: GTJA-0032
Frozen formula: (-1 * SUM(RANK(CORR(RANK(HIGH), RANK(VOLUME), 3)), 3))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0032"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0032"
ORIENTATION = 1.0
...
def build_pv_072_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_072_daily(daily_features)
    result = panel.merge(
        daily,
