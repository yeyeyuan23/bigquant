r"""PV-083: report-derived candidate, source GTJA-0045.

Source research ID: GTJA-0045
Frozen formula: (RANK(DELTA((((CLOSE * 0.6) + (OPEN *0.4))), 1)) * RANK(CORR(VWAP , MEAN(VOLUME,150), 15)))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0045"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0045"
ORIENTATION = 1.0
...
def build_pv_083_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_083_daily(daily_features)
    result = panel.merge(
        daily,
