r"""PV-146: report-derived candidate, source GTJA-0118.

Source research ID: GTJA-0118
Frozen formula: SUM(HIGH-OPEN,20)/SUM(OPEN-LOW,20)*100
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0118"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0118"
ORIENTATION = 1.0
...
def build_pv_146_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_146_daily(daily_features)
    result = panel.merge(
        daily,
