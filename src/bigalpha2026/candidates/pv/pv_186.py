r"""PV-186: report-derived candidate, source GTJA-0168.

Source research ID: GTJA-0168
Frozen formula: (-1*VOLUME/MEAN(VOLUME,20))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0168"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0168"
ORIENTATION = 1.0
...
def build_pv_186_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_186_daily(daily_features)
    result = panel.merge(
        daily,
