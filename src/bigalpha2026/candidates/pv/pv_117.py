r"""PV-117: report-derived candidate, source GTJA-0088.

Source research ID: GTJA-0088
Frozen formula: (CLOSE-DELAY(CLOSE,20))/DELAY(CLOSE,20)*100
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0088"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0088"
ORIENTATION = 1.0
...
def build_pv_117_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_117_daily(daily_features)
    result = panel.merge(
        daily,
