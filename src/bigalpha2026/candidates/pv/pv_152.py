r"""PV-152: report-derived candidate, source GTJA-0124.

Source research ID: GTJA-0124
Frozen formula: (CLOSE - VWAP) / DECAYLINEAR(RANK(TSMAX(CLOSE, 30)),2)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0124"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0124"
ORIENTATION = 1.0
...
def build_pv_152_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_152_daily(daily_features)
    result = panel.merge(
        daily,
