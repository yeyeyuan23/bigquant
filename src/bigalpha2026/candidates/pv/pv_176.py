r"""PV-176: report-derived candidate, source GTJA-0155.

Source research ID: GTJA-0155
Frozen formula: SMA(VOLUME,13,2)-SMA(VOLUME,27,2)-SMA(SMA(VOLUME,13,2)-SMA(VOLUME,27,2),10,2)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0155"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0155"
ORIENTATION = 1.0
...
def build_pv_176_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_176_daily(daily_features)
    result = panel.merge(
        daily,
