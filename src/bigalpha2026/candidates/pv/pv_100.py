r"""PV-100: report-derived candidate, source GTJA-0068.

Source research ID: GTJA-0068
Frozen formula: SMA(((HIGH+LOW)/2-(DELAY(HIGH,1)+DELAY(LOW,1))/2)*(HIGH-LOW)/VOLUME,15,2)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0068"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0068"
ORIENTATION = 1.0
...
def build_pv_100_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_100_daily(daily_features)
    result = panel.merge(
        daily,
