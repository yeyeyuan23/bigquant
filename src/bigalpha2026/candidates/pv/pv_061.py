r"""PV-061: report-derived candidate, source GTJA-0020.

Source research ID: GTJA-0020
Frozen formula: (CLOSE-DELAY(CLOSE,6))/DELAY(CLOSE,6)*100
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0020"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0020"
ORIENTATION = 1.0
...
def build_pv_061_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_061_daily(daily_features)
    result = panel.merge(
        daily,
