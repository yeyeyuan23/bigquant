r"""PV-088: report-derived candidate, source GTJA-0053.

Source research ID: GTJA-0053
Frozen formula: COUNT(CLOSE>DELAY(CLOSE,1),12)/12*100
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0053"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0053"
ORIENTATION = 1.0
...
def build_pv_088_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_088_daily(daily_features)
    result = panel.merge(
        daily,
