r"""PV-198: report-derived candidate, source GTJA-0188.

Source research ID: GTJA-0188
Frozen formula: ((HIGH-LOW–SMA(HIGH-LOW,11,2))/SMA(HIGH-LOW,11,2))*100
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0188"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0188"
ORIENTATION = 1.0
...
def build_pv_198_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_198_daily(daily_features)
    result = panel.merge(
        daily,
