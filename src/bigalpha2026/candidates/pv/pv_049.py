r"""PV-049: report-derived candidate, source GTJA-0005.

Source research ID: GTJA-0005
Frozen formula: (-1 * TSMAX(CORR(TSRANK(VOLUME, 5), TSRANK(HIGH, 5), 5), 3))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0005"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0005"
ORIENTATION = 1.0
...
def build_pv_049_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_049_daily(daily_features)
    result = panel.merge(
        daily,
