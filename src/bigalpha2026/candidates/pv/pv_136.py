r"""PV-136: report-derived candidate, source GTJA-0108.

Source research ID: GTJA-0108
Frozen formula: ((RANK((HIGH - MIN(HIGH, 2)))^RANK(CORR((VWAP), (MEAN(VOLUME,120)), 6))) * -1)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0108"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0108"
ORIENTATION = 1.0
...
def build_pv_136_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_136_daily(daily_features)
    result = panel.merge(
        daily,
