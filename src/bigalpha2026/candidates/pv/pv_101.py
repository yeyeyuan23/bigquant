r"""PV-101: report-derived candidate, source GTJA-0069.

Source research ID: GTJA-0069
Frozen formula: (SUM(DTM,20)>SUM(DBM,20) ？(SUM(DTM,20)-SUM(DBM,20))/SUM(DTM,20)：(SUM(DTM,20)=SUM(DBM,20)？ 0：(SUM(DTM,20)-SUM(DBM,20))/SUM(DBM,20)))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0069"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0069"
ORIENTATION = 1.0
...
def build_pv_101_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_101_daily(daily_features)
    result = panel.merge(
        daily,
