r"""PV-211: report-derived candidate, source HAITONG-0101.

Source research ID: HAITONG-0101
Frozen formula: max_42d(close)/min_42d(close)-1
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "HAITONG-0101"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "HAITONG-0101"
ORIENTATION = 1.0
...
def build_pv_211_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_211_daily(daily_features)
    result = panel.merge(
        daily,
