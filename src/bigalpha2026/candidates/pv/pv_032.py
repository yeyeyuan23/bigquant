r"""PV-032: Fangzheng report candidate, source FZ-057.

Source: 方正证券《球队硬币》, 16
Original formula: \(\mu_{20}(R^{co})\)
Frozen logic: 集合竞价隔夜价格变化
Data families: PV
...
Deduplication: FZ-053 at abs median daily Spearman 0.259962;
...
DATA_FAMILIES = ('PV',)
SOURCE_RESEARCH_ID = "FZ-057"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "FZ-057"
ORIENTATION = -1.0
...
def build_pv_032_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_032_daily(daily_features)
    result = panel.merge(
        daily,
