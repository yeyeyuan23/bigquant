r"""PV-059: report-derived candidate, source GTJA-0016.

Source research ID: GTJA-0016
Frozen formula: (-1 * TSMAX(RANK(CORR(RANK(VOLUME), RANK(VWAP), 5)), 5))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0016"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0016"
ORIENTATION = 1.0
...
def build_pv_059_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_059_daily(daily_features)
    result = panel.merge(
        daily,
