r"""PV-076: report-derived candidate, source GTJA-0038.

Source research ID: GTJA-0038
Frozen formula: (((SUM(HIGH, 20) / 20) < HIGH) ? (-1 * DELTA(HIGH, 2)) : 0)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0038"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0038"
ORIENTATION = 1.0
...
def build_pv_076_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_076_daily(daily_features)
    result = panel.merge(
        daily,
