r"""PV-075: report-derived candidate, source GTJA-0037.

Source research ID: GTJA-0037
Frozen formula: (-1 * RANK(((SUM(OPEN, 5) * SUM(RET, 5)) - DELAY((SUM(OPEN, 5) * SUM(RET, 5)), 10))))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0037"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0037"
ORIENTATION = 1.0
...
def build_pv_075_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_075_daily(daily_features)
    result = panel.merge(
        daily,
