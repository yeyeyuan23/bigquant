r"""PV-190: report-derived candidate, source GTJA-0172.

Source research ID: GTJA-0172
Frozen formula: MEAN(ABS(SUM((LD>0 & LD>HD)?LD:0,14)*100/ SUM(TR,14)-SUM((HD>0 & HD>LD)?HD:0,14)*100/SUM(TR,14))/(SUM((LD>0 & LD>HD)?LD:0,14)*100/ SUM(TR,14)+SUM((HD>0 & HD>LD)?HD:0,14)*100/SUM(TR,14))*100,6)
Economic logic: 短周期价格动态：以时序排名、差分、衰减和条件算子刻画动量/反转
Data families: PV
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0172"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0172"
ORIENTATION = 1.0
...
def build_pv_190_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_190_daily(daily_features)
    result = panel.merge(
        daily,
