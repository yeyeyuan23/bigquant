r"""PV-191: report-derived candidate, source GTJA-0176.

Source research ID: GTJA-0176
Frozen formula: CORR(RANK(((CLOSE - TSMIN(LOW, 12)) / (TSMAX(HIGH, 12) - TSMIN(LOW,12)))), RANK(VOLUME), 6)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0176"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0176"
ORIENTATION = 1.0
...
def build_pv_191_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_191_daily(daily_features)
    result = panel.merge(
        daily,
