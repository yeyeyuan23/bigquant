r"""PV-196: report-derived candidate, source GTJA-0185.

Source research ID: GTJA-0185
Frozen formula: RANK((-1 * ((1 - (OPEN / CLOSE))^2)))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0185"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0185"
ORIENTATION = 1.0
...
def build_pv_196_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_196_daily(daily_features)
    result = panel.merge(
        daily,
