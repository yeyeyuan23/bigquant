r"""PV-137: report-derived candidate, source GTJA-0109.

Source research ID: GTJA-0109
Frozen formula: SMA(HIGH-LOW,10,2)/SMA(SMA(HIGH-LOW,10,2),10,2)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0109"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0109"
ORIENTATION = 1.0
...
def build_pv_137_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_137_daily(daily_features)
    result = panel.merge(
        daily,
