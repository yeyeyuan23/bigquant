r"""PV-112: report-derived candidate, source GTJA-0083.

Source research ID: GTJA-0083
Frozen formula: (-1 * RANK(COVIANCE(RANK(HIGH), RANK(VOLUME), 5)))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0083"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0083"
ORIENTATION = 1.0
...
def build_pv_112_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_112_daily(daily_features)
    result = panel.merge(
        daily,
