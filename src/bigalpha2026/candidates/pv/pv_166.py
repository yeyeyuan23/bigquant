r"""PV-166: report-derived candidate, source GTJA-0140.

Source research ID: GTJA-0140
Frozen formula: MIN(RANK(DECAYLINEAR(((RANK(OPEN) + RANK(LOW)) - (RANK(HIGH) + RANK(CLOSE))), 8)), TSRANK(DECAYLINEAR(CORR(TSRANK(CLOSE, 8), TSRANK(MEAN(VOLUME,60), 20), 8), 7), 3))
Economic logic: 量价相关/背离：成交量与价格状态共同变化，捕捉需求确认或分歧
Data families: PV
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0140"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0140"
ORIENTATION = 1.0
...
def build_pv_166_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_166_daily(daily_features)
    result = panel.merge(
        daily,
