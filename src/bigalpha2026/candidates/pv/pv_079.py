r"""PV-079: report-derived candidate, source GTJA-0041.

Source research ID: GTJA-0041
Frozen formula: (RANK(MAX(DELTA((VWAP), 3), 5))* -1)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0041"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0041"
ORIENTATION = 1.0
...
def build_pv_079_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_079_daily(daily_features)
    result = panel.merge(
        daily,
