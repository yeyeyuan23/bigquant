r"""PV-171: report-derived candidate, source GTJA-0148.

Source research ID: GTJA-0148
Frozen formula: ((RANK(CORR((OPEN), SUM(MEAN(VOLUME,60), 9), 6)) < RANK((OPEN - TSMIN(OPEN, 14)))) * -1)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0148"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0148"
ORIENTATION = 1.0
...
def build_pv_171_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_171_daily(daily_features)
    result = panel.merge(
        daily,
