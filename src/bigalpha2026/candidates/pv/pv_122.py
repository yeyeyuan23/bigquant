r"""PV-122: report-derived candidate, source GTJA-0093.

Source research ID: GTJA-0093
Frozen formula: SUM((OPEN>=DELAY(OPEN,1)?0:MAX((OPEN-LOW),(OPEN-DELAY(OPEN,1)))),20)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0093"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0093"
ORIENTATION = 1.0
...
def build_pv_122_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_122_daily(daily_features)
    result = panel.merge(
        daily,
