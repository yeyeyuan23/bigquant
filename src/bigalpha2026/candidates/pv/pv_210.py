r"""PV-210: report-derived candidate, source HAITONG-0100.

Source research ID: HAITONG-0100
Frozen formula: max_21d(close)/min_21d(close)-1
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "HAITONG-0100"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "HAITONG-0100"
ORIENTATION = 1.0
...
def build_pv_210_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_210_daily(daily_features)
    result = panel.merge(
        daily,
