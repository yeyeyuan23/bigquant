r"""PV-116: report-derived candidate, source GTJA-0087.

Source research ID: GTJA-0087
Frozen formula: ((RANK(DECAYLINEAR(DELTA(VWAP , 4), 7)) + TSRANK(DECAYLINEAR(((((LOW * 0.9) + (LOW * 0.1)) - VWAP) / (OPEN - ((HIGH + LOW) / 2))), 11), 7)) * -1)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0087"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0087"
ORIENTATION = 1.0
...
def build_pv_116_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_116_daily(daily_features)
    result = panel.merge(
        daily,
