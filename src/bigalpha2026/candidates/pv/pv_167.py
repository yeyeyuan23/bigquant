r"""PV-167: report-derived candidate, source GTJA-0141.

Source research ID: GTJA-0141
Frozen formula: (RANK(CORR(RANK(HIGH), RANK(MEAN(VOLUME,15)), 9))* -1)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0141"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0141"
ORIENTATION = 1.0
...
def build_pv_167_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_167_daily(daily_features)
    result = panel.merge(
        daily,
