r"""PV-194: report-derived candidate, source GTJA-0179.

Source research ID: GTJA-0179
Frozen formula: (RANK(CORR(VWAP , VOLUME, 4)) *RANK(CORR(RANK(LOW), RANK(MEAN(VOLUME,50)), 12)))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0179"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0179"
ORIENTATION = 1.0
...
def build_pv_194_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_194_daily(daily_features)
    result = panel.merge(
        daily,
