r"""PV-071: report-derived candidate, source GTJA-0031.

Source research ID: GTJA-0031
Frozen formula: (CLOSE-MEAN(CLOSE,12))/MEAN(CLOSE,12)*100
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0031"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0031"
ORIENTATION = 1.0
...
def build_pv_071_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_071_daily(daily_features)
    result = panel.merge(
        daily,
