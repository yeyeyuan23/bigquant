r"""PV-038: Fangzheng report candidate, source FZ-089.

Source: 方正证券《飞蛾扑火》, 7
Original formula: \(\mu_{20}[(H_d-L_d)/C_{d-1}]\)
Frozen logic: 单日最大价格活动范围
Data families: PV
...
Deduplication: FZ-098 at abs median daily Spearman 0.899458;
...
DATA_FAMILIES = ('PV',)
SOURCE_RESEARCH_ID = "FZ-089"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "FZ-089"
ORIENTATION = -1.0
...
def build_pv_038_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_038_daily(daily_features)
    result = panel.merge(
        daily,
