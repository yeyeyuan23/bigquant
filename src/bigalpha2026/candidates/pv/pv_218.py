r"""PV-218: report-derived candidate, source CJ-G062-V03.

Source research ID: CJ-G062-V03
Frozen formula: corr_21d(daily volume,daily close)
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "CJ-G062-V03"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G062-V03"
ORIENTATION = 1.0
...
def build_pv_218_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_218_daily(daily_features)
    result = panel.merge(
        daily,
