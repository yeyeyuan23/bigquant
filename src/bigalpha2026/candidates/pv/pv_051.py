r"""PV-051: report-derived candidate, source GTJA-0007.

Source research ID: GTJA-0007
Frozen formula: ((RANK(MAX((VWAP - CLOSE), 3)) + RANK(MIN((VWAP - CLOSE), 3))) * RANK(DELTA(VOLUME, 3)))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0007"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0007"
ORIENTATION = 1.0
...
def build_pv_051_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_051_daily(daily_features)
    result = panel.merge(
        daily,
