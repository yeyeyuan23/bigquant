r"""PV-145: report-derived candidate, source GTJA-0117.

Source research ID: GTJA-0117
Frozen formula: ((TSRANK(VOLUME, 32) * (1 - TSRANK(((CLOSE + HIGH) - LOW), 16))) * (1 - TSRANK(RET, 32)))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0117"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0117"
ORIENTATION = 1.0
...
def build_pv_145_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_145_daily(daily_features)
    result = panel.merge(
        daily,
