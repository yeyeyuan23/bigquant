r"""PV-096: report-derived candidate, source GTJA-0062.

Source research ID: GTJA-0062
Frozen formula: (-1 * CORR(HIGH, RANK(VOLUME), 5))
...
DATA_FAMILIES = ("PV",)
SOURCE_RESEARCH_ID = "GTJA-0062"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "GTJA-0062"
ORIENTATION = 1.0
...
def build_pv_096_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_pv_096_daily(daily_features)
    result = panel.merge(
        daily,
