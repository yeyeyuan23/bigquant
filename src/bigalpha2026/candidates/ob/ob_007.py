r"""OB-007: report-derived candidate, source HAITONG-0145.

Source research ID: HAITONG-0145
Frozen formula: mean_20d(median_minute((bid_volume1-ask_volume1)/(bid_volume1+ask_volume1)))
...
DATA_FAMILIES = ("OB",)
SOURCE_RESEARCH_ID = "HAITONG-0145"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "HAITONG-0145"
ORIENTATION = 1.0
...
def build_ob_007_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_ob_007_daily(daily_features)
    result = panel.merge(
        daily,
