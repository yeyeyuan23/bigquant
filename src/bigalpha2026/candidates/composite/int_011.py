r"""INT-011: Fangzheng report candidate, source FZ-091.

Source: 方正证券《飞蛾扑火》, 8
Original formula: \(\mu_{20}(90)\)
Frozen logic: 分钟跳跃识别后的振幅
Data families: HF+PV
...
Deduplication: FZ-094 at abs median daily Spearman 0.884673;
...
def build_int_011_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_int_011_daily(daily_features)
    result = panel.merge(
        daily,
