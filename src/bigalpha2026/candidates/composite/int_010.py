r"""INT-010: Fangzheng report candidate, source FZ-090.

Source: 方正证券《飞蛾扑火》, 8
Original formula: 日跳跃度低于截面均值时取 \(-Amp_d\)，否则取 \(Amp_d\)
Frozen logic: 无跳跃“太阳”型振幅翻转，保留“火把”型跳跃风险
Data families: HF+PV
...
Deduplication: FZ-092 at abs median daily Spearman 0.493315;
...
def build_int_010_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_int_010_daily(daily_features)
    result = panel.merge(
        daily,
