r"""INT-008: Fangzheng report candidate, source FZ-063.

Source: 方正证券《球队硬币》, 21–22
Original formula: \(Orth(62\mid 风格+行业)\)
Frozen logic: 剥离流动性和波动率等传统暴露
Data families: PV+EXPOSURE
Semantic class: ANCHOR_COMPONENT
Fidelity: adapted_proxy
Data adaptation: inherits FACTORLIB turn proxy and reduced exposure set; available
  SIZE+LIQUIDTY+industry replace unspecified full style set
...
Deduplication: FZ-062 at abs median daily Spearman 0.758775;
...
def build_int_008_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_int_008_daily(daily_features)
    result = panel.merge(
        daily,
