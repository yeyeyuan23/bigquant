r"""INT-015: Fangzheng report candidate, source FZ-107.

Source: 方正证券《草木皆兵》, 8–9
Original formula: \(\mu_{20}(Retail_d\cdot Sal_d\cdot R_d)\)
Frozen logic: 小额个人交易占比高时更易情绪化过度反应
Data families: HF+PV
...
Deduplication: FZ-049 at abs median daily Spearman 0.761872;
...
def build_int_015_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_int_015_daily(daily_features)
    result = panel.merge(
        daily,
