r"""INT-009: Fangzheng report candidate, source FZ-084.

Source: 方正证券《云开雾散》, 14–15
Original formula: \(Orth(83\mid 风格+行业)\)
Frozen logic: 剥离流动性、波动率、估值、盈利等暴露
...
Deduplication: FZ-083 at abs median daily Spearman 0.751096;
...
def build_int_009_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_int_009_daily(daily_features)
    result = panel.merge(
        daily,
