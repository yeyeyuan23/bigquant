r"""INT-005: Fangzheng report candidate, source FZ-015.

Source: 方正证券《多空博弈》, 11–12
Original formula: \(Orth(14\mid 风格+行业)\)
Frozen logic: 剥离流动性、波动率等共同暴露后的增量信息
Data families: HF+EXPOSURE
Semantic class: ANCHOR_COMPONENT
Fidelity: adapted_proxy
Data adaptation: available SIZE+LIQUIDTY+industry replace unspecified full style set
...
Deduplication: FZ-014 at abs median daily Spearman 0.719761;
...
def build_int_005_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_int_005_daily(daily_features)
    result = panel.merge(
        daily,
