r"""INT-013: Fangzheng report candidate, source FZ-096.

Source: 方正证券《飞蛾扑火》, 12–13
Original formula: \(Orth(95\mid 风格+行业)\)
Frozen logic: 剥离流动性和波动率暴露
Data families: HF+PV+EXPOSURE
...
Deduplication: FZ-095 at abs median daily Spearman 0.772353;
...
def build_int_013_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_int_013_daily(daily_features)
    result = panel.merge(
        daily,
