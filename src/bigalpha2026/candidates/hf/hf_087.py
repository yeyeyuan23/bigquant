r"""HF-087: report-derived candidate, source CICC-027.

Source research ID: CICC-027
Frozen formula: minute_volume_share_skew/minute_volume_share_kurtosis
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CICC-027"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CICC-027"
ORIENTATION = 1.0
...
def build_hf_087_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_087_daily(daily_features)
    result = panel.merge(
        daily,
