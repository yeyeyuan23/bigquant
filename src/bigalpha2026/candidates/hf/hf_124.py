r"""HF-124: report-derived candidate, source CJ-G015-V03.

Source research ID: CJ-G015-V03
Frozen formula: mean_21d(non-opening 10m return; equal weighting)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G015-V03"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G015-V03"
ORIENTATION = 1.0
...
def build_hf_124_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_124_daily(daily_features)
    result = panel.merge(
        daily,
