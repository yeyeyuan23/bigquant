r"""HF-061: Fangzheng report candidate, source FZ-046.

Source: 方正证券《勇攀高峰》, 12
Original formula: 第 45 项日协方差的 20 日标准差
Frozen logic: 收盘波动率版本的稳定性
Data families: HF
...
Deduplication: FZ-043 at abs median daily Spearman 0.690455;
...
DATA_FAMILIES = ('HF',)
SOURCE_RESEARCH_ID = "FZ-046"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "FZ-046"
ORIENTATION = 1.0
...
def build_hf_061_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_061_daily(daily_features)
    result = panel.merge(
        daily,
