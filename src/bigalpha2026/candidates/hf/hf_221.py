r"""HF-221: report-derived candidate, source CJ-G105-V13.

Source research ID: CJ-G105-V13
Frozen formula: mean_21d((TWAP-low)/(high-low))
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G105-V13"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G105-V13"
ORIENTATION = 1.0
...
def build_hf_221_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_221_daily(daily_features)
    result = panel.merge(
        daily,
