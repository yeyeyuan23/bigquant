r"""HF-078: Fangzheng report candidate, source FZ-088.

Source: 方正证券《飞蛾扑火》, 5
Original formula: \(EW(86,87)\)
Frozen logic: 合成跳跃水平与稳定性
Data families: HF
...
Deduplication: FZ-087 at abs median daily Spearman 0.917599;
...
DATA_FAMILIES = ('HF',)
SOURCE_RESEARCH_ID = "FZ-088"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "FZ-088"
ORIENTATION = -1.0
...
def build_hf_078_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_078_daily(daily_features)
    result = panel.merge(
        daily,
