r"""HF-108: report-derived candidate, source CJ-G007-V01.

Source research ID: CJ-G007-V01
Frozen formula: mean_21d(10m low10% volume-segment return)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G007-V01"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G007-V01"
ORIENTATION = 1.0
...
def build_hf_108_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_108_daily(daily_features)
    result = panel.merge(
        daily,
