r"""HF-174: report-derived candidate, source CJ-G051-V03.

Source research ID: CJ-G051-V03
Frozen formula: mean_21d(5m price quintile 3 local statistic)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G051-V03"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G051-V03"
ORIENTATION = 1.0
...
def build_hf_174_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_174_daily(daily_features)
    result = panel.merge(
        daily,
