r"""HF-168: report-derived candidate, source CJ-G046-V05.

Source research ID: CJ-G046-V05
Frozen formula: mean_21d(5m pvol quintile 5 local statistic)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G046-V05"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G046-V05"
ORIENTATION = 1.0
...
def build_hf_168_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_168_daily(daily_features)
    result = panel.merge(
        daily,
