r"""HF-152: report-derived candidate, source CJ-G042-V03.

Source research ID: CJ-G042-V03
Frozen formula: mean_20d(volume peaks above mean+0sigma, min gap=5m)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G042-V03"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G042-V03"
ORIENTATION = 1.0
...
def build_hf_152_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_152_daily(daily_features)
    result = panel.merge(
        daily,
