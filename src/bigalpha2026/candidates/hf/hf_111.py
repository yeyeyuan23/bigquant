r"""HF-111: report-derived candidate, source CJ-G008-V05.

Source research ID: CJ-G008-V05
Frozen formula: mean_21d(30m high15% volume-segment return)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G008-V05"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G008-V05"
ORIENTATION = 1.0
...
def build_hf_111_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_111_daily(daily_features)
    result = panel.merge(
        daily,
