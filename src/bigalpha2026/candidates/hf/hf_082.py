r"""HF-082: report-derived candidate, source CICC-014.

Source research ID: CICC-014
Frozen formula: sum(log_return of 20 lowest-volume minutes; stable timestamp ties)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CICC-014"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CICC-014"
ORIENTATION = 1.0
...
def build_hf_082_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_082_daily(daily_features)
    result = panel.merge(
        daily,
