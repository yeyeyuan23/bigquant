r"""HF-128: report-derived candidate, source CJ-G028-V02.

Source research ID: CJ-G028-V02
Frozen formula: sum(amount*sign(ret))/sum(amount)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G028-V02"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G028-V02"
ORIENTATION = 1.0
...
def build_hf_128_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_128_daily(daily_features)
    result = panel.merge(
        daily,
