r"""HF-153: report-derived candidate, source CJ-G042-V04.

Source research ID: CJ-G042-V04
Frozen formula: mean_20d(volume peaks above mean+1sigma, min gap=0m)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G042-V04"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G042-V04"
ORIENTATION = 1.0
...
def build_hf_153_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_153_daily(daily_features)
    result = panel.merge(
        daily,
