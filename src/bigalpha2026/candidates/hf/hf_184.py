r"""HF-184: report-derived candidate, source CJ-G064-V03.

Source research ID: CJ-G064-V03
Frozen formula: mean_21d(log(first_minute_close/open))
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G064-V03"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G064-V03"
ORIENTATION = 1.0
...
def build_hf_184_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_184_daily(daily_features)
    result = panel.merge(
        daily,
