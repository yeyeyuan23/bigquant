r"""HF-037: Fangzheng report candidate, source FZ-001.

Source: 方正证券《多空博弈》, 4
Original formula: \(B_d(V,r^{(5m)})\)
Frozen logic: 成交量在强负/强正五分钟收益处的排序不对称，刻画多空博弈强弱
Data families: HF
...
Deduplication: RESERVED::corr_prv at abs median daily Spearman 0.770191;
...
DATA_FAMILIES = ('HF',)
SOURCE_RESEARCH_ID = "FZ-001"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "FZ-001"
ORIENTATION = -1.0
...
def build_hf_037_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_037_daily(daily_features)
    result = panel.merge(
        daily,
