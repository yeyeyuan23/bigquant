"""HF-026: competition-adapted CICC high-frequency candidate.

Source: CICC, High-Frequency Factor Handbook (2024-01-15), CICC-044.
Semantic class: LATENT_COMPONENT; include in self_library: true.
Data families: HF.
Frozen formula: Corr(one-minute close, one-minute volume growth).
Economic logic: Price-level alignment with activity acceleration.
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CICC-044"
SOURCE_FIDELITY = "exact"
COMPONENT_COLUMN = "corr_pvr"
ORIENTATION = -1.0
...
def compute_hf_026_daily(daily_features: pd.DataFrame) -> pd.DataFrame:
...
def build_hf_026_factor_from_daily(
    ...
    daily = compute_hf_026_daily(daily_features)
