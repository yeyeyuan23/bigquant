r"""HF-183: report-derived candidate, source CJ-G064-V02.

Source research ID: CJ-G064-V02
Frozen formula: mean_21d(log(first_minute_close/previous_close))
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G064-V02"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G064-V02"
ORIENTATION = 1.0
...
def build_hf_183_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_183_daily(daily_features)
    result = panel.merge(
        daily,
