r"""HF-088: report-derived candidate, source CICC-042.

Source research ID: CICC-042
Frozen formula: corr(close_t, volume_t+1), within session
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CICC-042"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CICC-042"
ORIENTATION = 1.0
...
def build_hf_088_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_088_daily(daily_features)
    result = panel.merge(
        daily,
