r"""HF-181: report-derived candidate, source CJ-G063-V02.

Source research ID: CJ-G063-V02
Frozen formula: std_21d(daily price-volume corr)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G063-V02"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G063-V02"
ORIENTATION = 1.0
...
def build_hf_181_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_181_daily(daily_features)
    result = panel.merge(
        daily,
