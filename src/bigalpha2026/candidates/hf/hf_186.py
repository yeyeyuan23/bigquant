r"""HF-186: report-derived candidate, source CJ-G065-V03.

Source research ID: CJ-G065-V03
Frozen formula: mean_21d(close-vs-first-minute-close return)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G065-V03"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G065-V03"
ORIENTATION = 1.0
...
def build_hf_186_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_186_daily(daily_features)
    result = panel.merge(
        daily,
