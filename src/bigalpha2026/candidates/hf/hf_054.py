r"""HF-054: Fangzheng report candidate, source FZ-039.

Source: 方正证券《勇攀高峰》, 5
Original formula: \(\mu_{20}(Cov_d)\)
Frozen logic: 波动升高而收益补偿下降，可能存在反应不足
Data families: HF
...
Deduplication: FZ-041 at abs median daily Spearman 0.886719;
...
DATA_FAMILIES = ('HF',)
SOURCE_RESEARCH_ID = "FZ-039"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "FZ-039"
ORIENTATION = 1.0
...
def build_hf_054_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_054_daily(daily_features)
    result = panel.merge(
        daily,
