r"""HF-079: report-derived candidate, source CICC-011.

Source research ID: CICC-011
Frozen formula: sum(log_return of 50 highest-volume minutes; stable timestamp ties)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CICC-011"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CICC-011"
ORIENTATION = 1.0
...
def build_hf_079_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_079_daily(daily_features)
    result = panel.merge(
        daily,
