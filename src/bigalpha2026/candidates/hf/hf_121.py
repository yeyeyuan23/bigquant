r"""HF-121: report-derived candidate, source CJ-G014-V04.

Source research ID: CJ-G014-V04
Frozen formula: mean_21d(opening 10m return; volume weighting)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G014-V04"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G014-V04"
ORIENTATION = 1.0
...
def build_hf_121_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_121_daily(daily_features)
    result = panel.merge(
        daily,
