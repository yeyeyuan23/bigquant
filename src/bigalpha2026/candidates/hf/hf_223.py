r"""HF-223: report-derived candidate, source CJ-G107-V02.

Source research ID: CJ-G107-V02
Frozen formula: mean_21d(slope(ret~amount/deal))
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G107-V02"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G107-V02"
ORIENTATION = 1.0
...
def build_hf_223_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_223_daily(daily_features)
    result = panel.merge(
        daily,
