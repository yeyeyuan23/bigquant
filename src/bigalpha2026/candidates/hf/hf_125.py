r"""HF-125: report-derived candidate, source CJ-G015-V04.

Source research ID: CJ-G015-V04
Frozen formula: mean_21d(non-opening 10m return; volume weighting)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G015-V04"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G015-V04"
ORIENTATION = 1.0
...
def build_hf_125_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_125_daily(daily_features)
    result = panel.merge(
        daily,
