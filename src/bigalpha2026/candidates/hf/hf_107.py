r"""HF-107: report-derived candidate, source CJ-G006-V03.

Source research ID: CJ-G006-V03
Frozen formula: mean_21d(sum(volume*ret)/sum(volume), 60m)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G006-V03"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G006-V03"
ORIENTATION = 1.0
...
def build_hf_107_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_107_daily(daily_features)
    result = panel.merge(
        daily,
