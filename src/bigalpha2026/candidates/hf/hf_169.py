r"""HF-169: report-derived candidate, source CJ-G048-V01.

Source research ID: CJ-G048-V01
Frozen formula: high20 pvol return - low20 pvol return
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G048-V01"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G048-V01"
ORIENTATION = 1.0
...
def build_hf_169_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_169_daily(daily_features)
    result = panel.merge(
        daily,
