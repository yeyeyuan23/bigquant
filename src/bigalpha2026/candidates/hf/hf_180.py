r"""HF-180: report-derived candidate, source CJ-G063-V01.

Source research ID: CJ-G063-V01
Frozen formula: skew_21d(daily hf reversal)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G063-V01"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G063-V01"
ORIENTATION = 1.0
...
def build_hf_180_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_180_daily(daily_features)
    result = panel.merge(
        daily,
