r"""HF-194: report-derived candidate, source CJ-G077-V01.

Source research ID: CJ-G077-V01
Frozen formula: mean_21d(high-volume pvol / all pvol)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G077-V01"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G077-V01"
ORIENTATION = 1.0
...
def build_hf_194_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_194_daily(daily_features)
    result = panel.merge(
        daily,
