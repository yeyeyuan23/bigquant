"""HF-028: competition-adapted CICC high-frequency candidate.

Source: CICC, High-Frequency Factor Handbook (2024-01-15), CICC-069.
Semantic class: LATENT_COMPONENT; include in J baseline: true.
Data families: HF.
Frozen formula: sum(last-50 one-minute return * one-minute volume share).
Economic logic: Volume-weighted directional pressure near the close.
...
Deduplication: Related to HF-004 but not the same residual BVC mechanism; observed correlation about 0.605.
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CICC-069"
SOURCE_FIDELITY = "exact"
COMPONENT_COLUMN = "trade_bottom50retRatio"
ORIENTATION = 1.0
...
def compute_hf_028_daily(daily_features: pd.DataFrame) -> pd.DataFrame:
...
def build_hf_028_factor_from_daily(
    ...
    daily = compute_hf_028_daily(daily_features)
