r"""HF-170: report-derived candidate, source CJ-G049-V01.

Source research ID: CJ-G049-V01
Frozen formula: sum((pvol-mean(pvol))*ret)/sum(pvol)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G049-V01"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G049-V01"
ORIENTATION = 1.0
...
def build_hf_170_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_170_daily(daily_features)
    result = panel.merge(
        daily,
