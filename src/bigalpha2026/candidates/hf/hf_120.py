r"""HF-120: report-derived candidate, source CJ-G014-V03.

Source research ID: CJ-G014-V03
Frozen formula: mean_21d(opening 10m return; equal weighting)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G014-V03"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G014-V03"
ORIENTATION = 1.0
...
def build_hf_120_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_120_daily(daily_features)
    result = panel.merge(
        daily,
