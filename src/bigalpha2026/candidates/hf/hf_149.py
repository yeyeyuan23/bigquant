r"""HF-149: report-derived candidate, source CJ-G040-V01.

Source research ID: CJ-G040-V01
Frozen formula: mean_20d(std(delta(V/N))/mean(V/N))
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G040-V01"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G040-V01"
ORIENTATION = 1.0
...
def build_hf_149_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_149_daily(daily_features)
    result = panel.merge(
        daily,
