r"""HF-187: report-derived candidate, source CJ-G066-V01.

Source research ID: CJ-G066-V01
Frozen formula: mean_21d(first5m volume/full volume)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G066-V01"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G066-V01"
ORIENTATION = 1.0
...
def build_hf_187_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_187_daily(daily_features)
    result = panel.merge(
        daily,
