r"""HF-058: Fangzheng report candidate, source FZ-043.

Source: 方正证券《勇攀高峰》, 8–9
Original formula: 上述日协方差的 \(\sigma_{20}\)
Frozen logic: 高波动补偿的稳定性；原始 IC 方向与均值项相反
Data families: HF
...
Deduplication: FZ-044 at abs median daily Spearman 0.731478;
...
DATA_FAMILIES = ('HF',)
SOURCE_RESEARCH_ID = "FZ-043"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "FZ-043"
ORIENTATION = 1.0
...
def build_hf_058_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_058_daily(daily_features)
    result = panel.merge(
        daily,
