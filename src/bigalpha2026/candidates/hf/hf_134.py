r"""HF-134: report-derived candidate, source CJ-G035-V01.

Source research ID: CJ-G035-V01
Frozen formula: entropy(normalized volume-share*relative-price)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G035-V01"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G035-V01"
ORIENTATION = 1.0
...
def build_hf_134_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_134_daily(daily_features)
    result = panel.merge(
        daily,
