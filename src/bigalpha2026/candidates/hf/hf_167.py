r"""HF-167: report-derived candidate, source CJ-G046-V04.

Source research ID: CJ-G046-V04
Frozen formula: mean_21d(5m pvol quintile 4 local statistic)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G046-V04"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G046-V04"
ORIENTATION = 1.0
...
def build_hf_167_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_167_daily(daily_features)
    result = panel.merge(
        daily,
