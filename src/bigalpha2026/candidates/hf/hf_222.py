r"""HF-222: report-derived candidate, source CJ-G106-V01.

Source research ID: CJ-G106-V01
Frozen formula: mean_21d(rank corr(ret,volume))
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G106-V01"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G106-V01"
ORIENTATION = 1.0
...
def build_hf_222_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_222_daily(daily_features)
    result = panel.merge(
        daily,
