r"""HF-084: report-derived candidate, source CICC-019.

Source research ID: CICC-019
Frozen formula: up_vol/(up_vol+down_vol)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CICC-019"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CICC-019"
ORIENTATION = 1.0
...
def build_hf_084_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_084_daily(daily_features)
    result = panel.merge(
        daily,
