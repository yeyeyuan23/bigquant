r"""HF-190: report-derived candidate, source CJ-G071-V01.

Source research ID: CJ-G071-V01
Frozen formula: corr_21d(daily intraday volatility, daily mean price)
...
DATA_FAMILIES = ("HF",)
SOURCE_RESEARCH_ID = "CJ-G071-V01"
SOURCE_FIDELITY = "formalized_from_report"
COMPONENT_COLUMN = "CJ-G071-V01"
ORIENTATION = 1.0
...
def build_hf_190_factor_from_daily(
    daily_features: pd.DataFrame,
    pool: pd.DataFrame,
) -> pd.DataFrame:
    ...
    daily = compute_hf_190_daily(daily_features)
    result = panel.merge(
        daily,
