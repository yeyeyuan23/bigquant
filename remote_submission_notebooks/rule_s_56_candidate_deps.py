...
# ---- bigalpha2026.candidates.pv.pv_024 ----
_ba_candidates__pv__pv_024__CANDIDATE_ID = 'PV-024'
_ba_candidates__pv__pv_024__SEMANTIC_CLASS = 'LATENT_COMPONENT'
_ba_candidates__pv__pv_024__INCLUDE_IN_J_BASELINE = True
_ba_candidates__pv__pv_024__DATA_FAMILIES = ('PV',)
_ba_candidates__pv__pv_024__SOURCE_RESEARCH_ID = 'FZ-049'
_ba_candidates__pv__pv_024__SOURCE_FIDELITY = 'formalized_from_report'
_ba_candidates__pv__pv_024__COMPONENT_COLUMN = 'FZ-049'
_ba_candidates__pv__pv_024__ORIENTATION = -1.0
_ba_candidates__pv__pv_024__POOL_COLUMNS = ('date', 'instrument')
_ba_candidates__pv__pv_024__OUTPUT_COLUMNS = ('date', 'instrument', 'factor')

def _ba_candidates__pv__pv_024___require_columns(frame: pd.DataFrame, columns: Iterable[str], name: str) -> None:
    missing = sorted(set(columns).difference(frame.columns))
    if missing:
        raise ValueError(f'{name} is missing required columns: {missing}')

def _ba_candidates__pv__pv_024__compute_pv_024_daily(daily_features: pd.DataFrame) -> pd.DataFrame:
    """Read the frozen report state without changing its definition."""
    required = (*_ba_candidates__pv__pv_024__POOL_COLUMNS, _ba_candidates__pv__pv_024__COMPONENT_COLUMN)
    _ba_candidates__pv__pv_024___require_columns(daily_features, required, 'daily_features')
    daily = daily_features.loc[:, required].copy()
    daily['date'] = pd.to_datetime(daily['date'], errors='coerce').dt.normalize()
    daily['instrument'] = daily['instrument'].astype(str)
    if daily.duplicated(list(_ba_candidates__pv__pv_024__POOL_COLUMNS)).any():
        raise ValueError('daily_features contains duplicate date-instrument keys')
    daily['factor_raw'] = pd.to_numeric(daily[_ba_candidates__pv__pv_024__COMPONENT_COLUMN], errors='coerce')
...
def _ba_candidates__pv__pv_024__build_pv_024_factor_from_daily(daily_features: pd.DataFrame, pool: pd.DataFrame) -> pd.DataFrame:
    """Build the oriented three-column candidate from the frozen daily state."""
    _ba_candidates__pv__pv_024___require_columns(pool, _ba_candidates__pv__pv_024__POOL_COLUMNS, 'pool')
    panel = pool.loc[:, _ba_candidates__pv__pv_024__POOL_COLUMNS].copy()
    panel['date'] = pd.to_datetime(panel['date'], errors='coerce').dt.normalize()
    panel['instrument'] = panel['instrument'].astype(str)
    panel = panel.dropna(subset=list(_ba_candidates__pv__pv_024__POOL_COLUMNS))
    if panel.duplicated(list(_ba_candidates__pv__pv_024__POOL_COLUMNS)).any():
        raise ValueError('pool contains duplicate date-instrument keys')
    daily = _ba_candidates__pv__pv_024__compute_pv_024_daily(daily_features)
    result = panel.merge(daily, on=list(_ba_candidates__pv__pv_024__POOL_COLUMNS), how='left', validate='one_to_one')
    result['factor'] = _ba_candidate_transforms__daily_median_centered_rank(result, orientation=_ba_candidates__pv__pv_024__ORIENTATION)
    result['factor'] = result['factor'].replace([np.inf, -np.inf], np.nan)
    if result['factor'].isna().any():
        raise ValueError(f'{_ba_candidates__pv__pv_024__CANDIDATE_ID} produced non-finite factor values')
    return result.loc[:, _ba_candidates__pv__pv_024__OUTPUT_COLUMNS].sort_values(['date', 'instrument']).reset_index(drop=True)
...
# ---- bigalpha2026.candidates.pv.pv_035 ----
_ba_candidates__pv__pv_035__CANDIDATE_ID = 'PV-035'
_ba_candidates__pv__pv_035__SEMANTIC_CLASS = 'LATENT_COMPONENT'
_ba_candidates__pv__pv_035__INCLUDE_IN_J_BASELINE = True
_ba_candidates__pv__pv_035__DATA_FAMILIES = ('PV',)
_ba_candidates__pv__pv_035__SOURCE_RESEARCH_ID = 'FZ-060'
_ba_candidates__pv__pv_035__SOURCE_FIDELITY = 'adapted_proxy'
_ba_candidates__pv__pv_035__COMPONENT_COLUMN = 'FZ-060'
_ba_candidates__pv__pv_035__ORIENTATION = -1.0
_ba_candidates__pv__pv_035__POOL_COLUMNS = ('date', 'instrument')
_ba_candidates__pv__pv_035__OUTPUT_COLUMNS = ('date', 'instrument', 'factor')

def _ba_candidates__pv__pv_035___require_columns(frame: pd.DataFrame, columns: Iterable[str], name: str) -> None:
    missing = sorted(set(columns).difference(frame.columns))
    if missing:
        raise ValueError(f'{name} is missing required columns: {missing}')

def _ba_candidates__pv__pv_035__compute_pv_035_daily(daily_features: pd.DataFrame) -> pd.DataFrame:
    """Read the frozen report state without changing its definition."""
    required = (*_ba_candidates__pv__pv_035__POOL_COLUMNS, _ba_candidates__pv__pv_035__COMPONENT_COLUMN)
    _ba_candidates__pv__pv_035___require_columns(daily_features, required, 'daily_features')
    daily = daily_features.loc[:, required].copy()
    daily['date'] = pd.to_datetime(daily['date'], errors='coerce').dt.normalize()
    daily['instrument'] = daily['instrument'].astype(str)
    if daily.duplicated(list(_ba_candidates__pv__pv_035__POOL_COLUMNS)).any():
        raise ValueError('daily_features contains duplicate date-instrument keys')
    daily['factor_raw'] = pd.to_numeric(daily[_ba_candidates__pv__pv_035__COMPONENT_COLUMN], errors='coerce')
    daily['factor_raw'] = daily['factor_raw'].replace([np.inf, -np.inf], np.nan)
    return daily[['date', 'instrument', 'factor_raw']].sort_values(['date', 'instrument']).reset_index(drop=True)

def _ba_candidates__pv__pv_035__build_pv_035_factor_from_daily(daily_features: pd.DataFrame, pool: pd.DataFrame) -> pd.DataFrame:
    """Build the oriented three-column candidate from the frozen daily state."""
    _ba_candidates__pv__pv_035___require_columns(pool, _ba_candidates__pv__pv_035__POOL_COLUMNS, 'pool')
    panel = pool.loc[:, _ba_candidates__pv__pv_035__POOL_COLUMNS].copy()
    panel['date'] = pd.to_datetime(panel['date'], errors='coerce').dt.normalize()
    panel['instrument'] = panel['instrument'].astype(str)
    panel = panel.dropna(subset=list(_ba_candidates__pv__pv_035__POOL_COLUMNS))
    if panel.duplicated(list(_ba_candidates__pv__pv_035__POOL_COLUMNS)).any():
        raise ValueError('pool contains duplicate date-instrument keys')
    daily = _ba_candidates__pv__pv_035__compute_pv_035_daily(daily_features)
    result = panel.merge(daily, on=list(_ba_candidates__pv__pv_035__POOL_COLUMNS), how='left', validate='one_to_one')
    result['factor'] = _ba_candidate_transforms__daily_median_centered_rank(result, orientation=_ba_candidates__pv__pv_035__ORIENTATION)
    result['factor'] = result['factor'].replace([np.inf, -np.inf], np.nan)
    if result['factor'].isna().any():
        raise ValueError(f'{_ba_candidates__pv__pv_035__CANDIDATE_ID} produced non-finite factor values')
    return result.loc[:, _ba_candidates__pv__pv_035__OUTPUT_COLUMNS].sort_values(['date', 'instrument']).reset_index(drop=True)
...
# ---- bigalpha2026.candidates.hf.hf_021 ----
_ba_candidates__hf__hf_021__CANDIDATE_ID = 'HF-021'
_ba_candidates__hf__hf_021__SEMANTIC_CLASS = 'LATENT_COMPONENT'
_ba_candidates__hf__hf_021__INCLUDE_IN_J_BASELINE = True
_ba_candidates__hf__hf_021__DATA_FAMILIES = ('HF',)
_ba_candidates__hf__hf_021__SOURCE_RESEARCH_ID = 'CICC-028'
_ba_candidates__hf__hf_021__SOURCE_FIDELITY = 'exact'
_ba_candidates__hf__hf_021__COMPONENT_COLUMN = 'liq_amihud_1min'
_ba_candidates__hf__hf_021__ORIENTATION = 1.0
_ba_candidates__hf__hf_021__POOL_COLUMNS = ('date', 'instrument')
_ba_candidates__hf__hf_021__OUTPUT_COLUMNS = ('date', 'instrument', 'factor')

def _ba_candidates__hf__hf_021___require_columns(frame: pd.DataFrame, columns: Iterable[str], name: str) -> None:
    missing = sorted(set(columns).difference(frame.columns))
    if missing:
        raise ValueError(f'{name} is missing required columns: {missing}')

def _ba_candidates__hf__hf_021__compute_hf_021_daily(daily_features: pd.DataFrame) -> pd.DataFrame:
    """Read the frozen raw daily component without changing its definition."""
    required = (*_ba_candidates__hf__hf_021__POOL_COLUMNS, _ba_candidates__hf__hf_021__COMPONENT_COLUMN)
    _ba_candidates__hf__hf_021___require_columns(daily_features, required, 'daily_features')
    daily = daily_features.loc[:, required].copy()
    daily['date'] = pd.to_datetime(daily['date'], errors='coerce').dt.normalize()
    daily['instrument'] = daily['instrument'].astype(str)
    if daily.duplicated(list(_ba_candidates__hf__hf_021__POOL_COLUMNS)).any():
        raise ValueError('daily_features contains duplicate date-instrument keys')
    daily['factor_raw'] = pd.to_numeric(daily[_ba_candidates__hf__hf_021__COMPONENT_COLUMN], errors='coerce')
    daily['factor_raw'] = daily['factor_raw'].replace([np.inf, -np.inf], np.nan)
    return daily[['date', 'instrument', 'factor_raw']].sort_values(['date', 'instrument']).reset_index(drop=True)

def _ba_candidates__hf__hf_021__build_hf_021_factor_from_daily(daily_features: pd.DataFrame, pool: pd.DataFrame) -> pd.DataFrame:
    """Build the oriented three-column candidate from the frozen daily panel."""
    _ba_candidates__hf__hf_021___require_columns(pool, _ba_candidates__hf__hf_021__POOL_COLUMNS, 'pool')
    panel = pool.loc[:, _ba_candidates__hf__hf_021__POOL_COLUMNS].copy()
    panel['date'] = pd.to_datetime(panel['date'], errors='coerce').dt.normalize()
    panel['instrument'] = panel['instrument'].astype(str)
    panel = panel.dropna(subset=list(_ba_candidates__hf__hf_021__POOL_COLUMNS))
    if panel.duplicated(list(_ba_candidates__hf__hf_021__POOL_COLUMNS)).any():
        raise ValueError('pool contains duplicate date-instrument keys')
    daily = _ba_candidates__hf__hf_021__compute_hf_021_daily(daily_features)
    result = panel.merge(daily, on=list(_ba_candidates__hf__hf_021__POOL_COLUMNS), how='left', validate='one_to_one')
    result['factor'] = _ba_candidate_transforms__daily_median_centered_rank(result, orientation=_ba_candidates__hf__hf_021__ORIENTATION)
    result['factor'] = result['factor'].replace([np.inf, -np.inf], np.nan)
    if result['factor'].isna().any():
        raise ValueError(f'{_ba_candidates__hf__hf_021__CANDIDATE_ID} produced non-finite factor values')
    return result.loc[:, _ba_candidates__hf__hf_021__OUTPUT_COLUMNS].sort_values(['date', 'instrument']).reset_index(drop=True)
...
CANDIDATE_SPECS = {
    ...
    'HF-021': (_ba_candidates__hf__hf_021__build_hf_021_factor_from_daily, _ba_candidates__hf__hf_021__COMPONENT_COLUMN, _ba_candidates__hf__hf_021__ORIENTATION),
    ...
    'PV-024': (_ba_candidates__pv__pv_024__build_pv_024_factor_from_daily, _ba_candidates__pv__pv_024__COMPONENT_COLUMN, _ba_candidates__pv__pv_024__ORIENTATION),
    ...
    'PV-035': (_ba_candidates__pv__pv_035__build_pv_035_factor_from_daily, _ba_candidates__pv__pv_035__COMPONENT_COLUMN, _ba_candidates__pv__pv_035__ORIENTATION),
