"""Strict-OOS temporal learner for one or more frozen OOS residual targets."""
...
def residualize_labels(
    labels: pd.DataFrame,
    residual_routes: list[tuple[str, Path, float]],
) -> tuple[pd.DataFrame, dict[str, object]]:
    """Build a target residual using only supplied frozen OOS predictions."""
    ...
    raw_target = pd.to_numeric(
        frame["ret_next_open_to_close"], errors="coerce"
    )
    frame["_target_rank"] = (
        raw_target.groupby(frame["date"]).rank(method="average", pct=True)
        * 2.0
        - 1.0
    )
    lineage: list[dict[str, object]] = []
    route_columns: list[tuple[str, float]] = []
    for index, (name, path, weight) in enumerate(residual_routes):
        route = pd.read_parquet(path)
        if list(route.columns) != ["date", "instrument", "factor"]:
            raise ValueError(f"residual route {name} has invalid columns")
        route["date"] = pd.to_datetime(
            route["date"], errors="coerce"
        ).dt.normalize()
        route["instrument"] = route["instrument"].astype(str)
        if route[keys].isna().any().any() or route.duplicated(keys).any():
            raise ValueError(f"residual route {name} has invalid keys")
        route["factor"] = pd.to_numeric(route["factor"], errors="coerce")
        if not np.isfinite(route["factor"]).all():
            raise ValueError(f"residual route {name} contains non-finite values")
        column = f"_residual_route_{index}"
        route[column] = (
            route.groupby("date")["factor"].rank(method="average", pct=True)
            * 2.0
            - 1.0
        )
        frame = frame.merge(
            route[keys + [column]], on=keys, how="left", validate="one_to_one"
        )
        route_columns.append((column, weight))
        lineage.append(
            {
                "name": name,
                "path": str(path),
    ...
    residual = frame["_target_rank"].copy()
    for column, weight in route_columns:
        residual = residual - weight * frame[column]
    complete = frame[[column for column, _ in route_columns]].notna().all(axis=1)
    frame["ret_next_open_to_close"] = residual.where(complete)
    return frame.drop(
        columns=["_target_rank", *[column for column, _ in route_columns]]
    ), {"routes": lineage, "eligible_rows": int(complete.sum())}
...
def evaluate_fold(
    ...
    *,
    ...
    residual_routes: list[tuple[str, Path, float]] | None = None,
) -> tuple[pd.DataFrame, dict[str, object]]:
    ...
    residual_lineage: dict[str, object] | None = None
    if residual_routes:
        labels, residual_lineage = residualize_labels(labels, residual_routes)
    ...
    metrics = {
        ...
        "target_role": "strict_oos_residual",
...
def main() -> int:
    ...
    residual_routes = [
        (name, Path(path), float(weight))
        for name, path, weight in args.residual_route
    ]
    candidate_dim = len(
        candidate_ids_from_manifest(
