...
DAILY_PATH = WORK / f"data/daily_ohlcv_adjusted_{PERIOD_TAG}.parquet"
OUTPUT_DIR = WORK / "haitong"
PRIMITIVE_PATH = OUTPUT_DIR / f"haitong_minute_primitives_{PERIOD_TAG}.parquet"
PANEL_PATH = OUTPUT_DIR / f"haitong_raw_panel_{PERIOD_TAG}.parquet"
RESULT_PATH = OUTPUT_DIR / "haitong_sandbox_results.csv"
SUMMARY_PATH = OUTPUT_DIR / "haitong_sandbox_summary.json"
SUBMISSION_MANIFEST = Path(__file__).with_name("submission_manifest_haitong_11.csv")
...
def _frequency_returns(frame: pd.DataFrame, frequency: int, offset: int = 0) -> pd.DataFrame:
    selected = frame.loc[
        ((frame["minute_index"] - offset) % frequency).eq(0),
        ["date", "instrument_id", "session", "timestamp", "close"],
    ].copy()
    selected["ret"] = (
        np.log(selected["close"])
        .groupby(
            [
...
def build_month(path: Path) -> pd.DataFrame:
    ...
    frame["minute_index"] = (
        frame.groupby(["instrument_id", "date", "session"], sort=False).cumcount() + 1
    )
    ...
    for offset in range(5):
        part = _moment_stats(_frequency_returns(frame, 5, offset=offset), f"offset{offset}")
        rename = {
...
def _rolling_mean(frame: pd.DataFrame, series: pd.Series, window: int) -> pd.Series:
    result = (
        series.groupby(frame["instrument"], sort=False)
        .rolling(window, min_periods=max(5, int(np.ceil(window * 0.75))))
        .mean()
    )
...
def _rolling_max(frame: pd.DataFrame, series: pd.Series, window: int) -> pd.Series:
    result = (
        series.groupby(frame["instrument"], sort=False)
        .rolling(window, min_periods=max(5, int(np.ceil(window * 0.75))))
        .max()
...
def _rolling_min(frame: pd.DataFrame, series: pd.Series, window: int) -> pd.Series:
    result = (
        series.groupby(frame["instrument"], sort=False)
        .rolling(window, min_periods=max(5, int(np.ceil(window * 0.75))))
        .min()
...
def build_factor_panel(base: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, str]]:
    ...
    for source_id, months in zip(
        ...
        window = months * 21
        value = (
            _rolling_max(base, base["close"], window) / _rolling_min(base, base["close"], window)
            - 1.0
        )
    ...
    add(
        ...
        _rolling_mean(
            base,
            base["tail60_amount"] / base["total_amount"].where(base["total_amount"].gt(0)),
            20,
        ),
...
def main() -> None:
    ...
    if missing_columns:
        raise RuntimeError(
            f"Haitong generator did not emit submitted components: {missing_columns}"
        )
    ...
    for source_id, formula in formulas.items():
        ...
        status = "TECHNICAL_PASS" if coverage >= 0.25 and unique >= 2 else "TECHNICAL_FAIL"
        rows.append(
            {
                "source_id": source_id,
