...
Monthly inputs may be either `YYYYMM.0.parquet` or `YYYYMM.0.feather`; the
reader rejects a month if both formats are present instead of choosing one
silently. The instrument map must cover every raw `instrument_id`.
...
The Changjiang minute stage can use independent monthly workers without
changing formulas or output order:
...
## Remaining-132 model matrix and temporal validation

After the Changjiang and Haitong panels exist, build the exact remaining-132
wide feature matrix plus a separate raw-availability mask. Missing raw
components are never backfilled across dates.

```bash
python scripts/factor_wiki_remaining/build_remaining132_feature_matrix.py \
  --work /path/to/generated/factor_wiki_remaining \
  --candidate-pool /path/to/candidate_pool.parquet
```

Run a full-history versus physical-cutoff rebuild and compare every common
prefix row for all 132 candidates:

```bash
python scripts/factor_wiki_remaining/validate_prefix_invariance.py \
  --work /path/to/generated/factor_wiki_remaining \
  --cutoff 2020-12-31
```
