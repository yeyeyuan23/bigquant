...
try:
    ...
    from io_utils import discover_month_paths, read_columns
...
CICC_DIRECT = Path(
    os.environ.get(
        ...
        str(
            PROJECT_ROOT / "入库前沙盒结果/CICC_79_local_direct_remaining19_v1/data/"
...
CICC_PILOT = Path(
    os.environ.get(
        ...
        str(
            PROJECT_ROOT / "入库前沙盒结果/CICC_79_local_direct_pilot_v1/data/"
...
def pair_corr(frame: pd.DataFrame, left: str, right: str, output: str) -> pd.DataFrame:
    ...
    stats = stats.drop(columns="sxy").merge(xy, on=["date", "instrument_id"], validate="one_to_one")
...
def build_month(path: Path) -> pd.DataFrame:
    frame = read_columns(path, ["date", "instrument_id", "close", "volume"])
    ...
    frame["volume_lead"] = frame["volume"].groupby(session_keys, sort=False).shift(-1)
    frame["volume_lag"] = frame["volume"].groupby(session_keys, sort=False).shift(1)
    frame["day_position"] = frame.groupby(["instrument_id", "date"], sort=False).cumcount() + 1
    frame["volume_rank_desc"] = frame.groupby(["instrument_id", "date"], sort=False)["volume"].rank(
        method="first", ascending=False
    )
    frame["volume_rank_asc"] = frame.groupby(["instrument_id", "date"], sort=False)["volume"].rank(
        method="first", ascending=True
    )
    frame["total_volume"] = frame.groupby(["instrument_id", "date"], sort=False)[
        "volume"
    ].transform("sum")
    frame["q"] = frame["volume"] / frame["total_volume"].where(frame["total_volume"].gt(0))

    for count in (20, 50):
        frame[f"top{count}_ret"] = frame["ret"].where(frame["volume_rank_desc"].le(count), 0.0)
        frame[f"bottom{count}_ret"] = frame["ret"].where(frame["volume_rank_asc"].le(count), 0.0)
    first20 = frame["day_position"].le(20)
    frame["first20_weighted_ret"] = (frame["ret"] * frame["q"]).where(first20, 0.0)
    frame["first20_negative"] = (frame["ret"].abs() * frame["q"]).where(
        first20 & frame["ret"].lt(0), 0.0
    )
    frame["first20_positive"] = (frame["ret"] * frame["q"]).where(first20 & frame["ret"].gt(0), 0.0)
    daily = (
    ...
    for part in (
        ...
        daily = daily.merge(part, on=["date", "instrument_id"], how="left", validate="one_to_one")
    return daily
...
def main() -> None:
    ...
    if RAW_DAILY_PATH.exists():
        ...
        paths = discover_month_paths(RAW_DIR, (2019, 2020, 2021))
        parts = []
        ...
        raw_daily = raw_daily.merge(mapping, on="instrument_id", how="left", validate="many_to_one")
    ...
    base = (
        raw_daily.merge(primitives, on=["date", "instrument"], how="outer", validate="one_to_one")
        .merge(direct, on=["date", "instrument"], how="outer", validate="one_to_one")
        .merge(pilot, on=["date", "instrument"], how="outer", validate="one_to_one")
    )
    ...
    if missing_submission_columns:
        raise RuntimeError(
            f"CICC submission components were not generated: {missing_submission_columns}"
        )
