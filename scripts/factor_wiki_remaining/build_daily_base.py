...
OUTPUT_PATH = OUTPUT_DIR / f"daily_ohlcv_adjusted_{PERIOD_TAG}.parquet"
MANIFEST_PATH = OUTPUT_DIR / f"daily_ohlcv_adjusted_{PERIOD_TAG}.manifest.json"
READ_COLUMNS = (
    "date",
    "instrument_id",
    "adjust_factor",
...
def build_month(path: Path) -> pd.DataFrame:
    frame = read_columns(path, READ_COLUMNS)
    ...
    frame["adjust_factor"] = pd.to_numeric(frame["adjust_factor"], errors="coerce").astype(float)
    ...
    frame["amount"] = (raw_amount / 100.0).where(~(raw_amount.eq(0) & frame["volume"].gt(0)))
    frame["deal_number"] = pd.to_numeric(frame["deal_number"], errors="coerce").where(
        lambda value: value.ge(0)
    )
    ...
    daily["vwap_raw"] = daily["amount"].div(daily["volume"].where(daily["volume"].gt(0)))
    for output, raw in (
        ("open", "open_raw"),
        ("high", "high_raw"),
        ("low", "low_raw"),
...
def main() -> None:
    ...
    source_instruments = int(daily["instrument_id"].nunique())
    daily = daily.merge(mapping, on="instrument_id", how="inner", validate="many_to_one")
    if daily.empty:
        raise ValueError("instrument mapping removed every source row")
    daily = daily.sort_values(["instrument", "date"], kind="mergesort").reset_index(drop=True)
    ...
    manifest = {
        ...
        "source_rows_before_universe_filter": source_rows,
        "source_instruments_before_universe_filter": source_instruments,
        "universe_filter": str(MAPPING_PATH),
