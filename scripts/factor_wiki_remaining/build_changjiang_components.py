...
except ModuleNotFoundError:  # Direct execution from this script directory.
    from io_utils import configured_years, discover_month_paths, period_tag, read_columns

REPO_ROOT = Path(__file__).resolve().parents[2]
PROJECT_ROOT = Path(os.environ.get("BIGALPHA_PROJECT_ROOT", str(REPO_ROOT.parent))).expanduser()
WORK = Path(
    os.environ.get(
...
MAPPING_PATH = Path(
    os.environ.get(
        ...
        str(
            PROJECT_ROOT / "Data_wiki/artifacts/"
            "bigalpha_2026_stock_bar1m_instrument_map_2019_2024.csv"
        ),
    )
).expanduser()
ATOMIZED = Path(__file__).with_name("changjiang_static_dedup_352.csv")
SUBMISSION_MANIFEST = Path(__file__).with_name("submission_manifest_changjiang_123.csv")
OUTPUT_DIR = WORK / "changjiang"
YEARS = configured_years()
PERIOD_TAG = period_tag(YEARS)
PRIMITIVE_PATH = OUTPUT_DIR / f"changjiang_daily_primitives_{PERIOD_TAG}.parquet"
PANEL_PATH = OUTPUT_DIR / f"changjiang_raw_panel_{PERIOD_TAG}.parquet"
...
def _corr_columns(frame: pd.DataFrame, left: str, right: str, name: str) -> pd.DataFrame:
...
def _slope_columns(frame: pd.DataFrame, left: str, right: str, name: str) -> pd.DataFrame:
...
def _prepare_one_minute(path: Path) -> pd.DataFrame:
    ...
    frame = frame.sort_values(["instrument_id", "timestamp"], kind="mergesort").reset_index(
        drop=True
    )
    ...
    frame["amplitude"] = (frame["high"] - frame["low"]) / frame["open"].where(frame["open"].gt(EPS))
...
def _aggregate_bars(frame: pd.DataFrame, frequency: int) -> pd.DataFrame:
    ...
    bars["amplitude"] = (bars["high"] - bars["low"]) / bars["open"].where(bars["open"].gt(EPS))
    ...
    bars["density"] = np.log(
        bars["absret"].where(bars["absret"].gt(EPS)) / bars["volume"].where(bars["volume"].gt(0))
    )
...
def _daily_features(bars: pd.DataFrame, prefix: str) -> pd.DataFrame:
    ...
    core["pvol_centered_vwret"] = core["pvol_centered_ret"] / pd.Series(pvol_sum).where(
        pd.Series(pvol_sum).abs().gt(EPS)
    )
    core["pvol_vwret"] = core["pvol_ret"] / pd.Series(pvol_sum).where(
        pd.Series(pvol_sum).abs().gt(EPS)
    )
    core["absret_vwret"] = core["absret_ret"] / core["absret_sum"].where(core["absret_sum"].gt(EPS))
    core["absret_weighted_close"] = (
        core["absret_close"] / core["absret_sum"].where(core["absret_sum"].gt(EPS))
    ...
    core["deal_cv"] = core["deal_std"] / core["deal_mean"].where(core["deal_mean"].abs().gt(EPS))
    core["amp_cv"] = core["amp_std"] / core["amp_mean"].where(core["amp_mean"].abs().gt(EPS))
    core["pvol_diff_cv"] = np.nan
    core["illiq_cv"] = core["illiq_std"] / core["illiq_mean"].where(
        core["illiq_mean"].abs().gt(EPS)
    )
    core["up_ratio"] = core["up2"] / core["ret2_sum"].where(core["ret2_sum"].gt(EPS))
    core["down_ratio"] = core["down2"] / core["ret2_sum"].where(core["ret2_sum"].gt(EPS))
    core["weighted_close_ratio"] = (
        core["volume_close"] / core["volume_sum"].where(core["volume_sum"].gt(0))
    ) / core["close_mean"].where(core["close_mean"].gt(EPS))
    ...
    core["pamount_quantile_ratio"] = (core["pamount_q15"] - core["pamount_min"]) / (
        core["pamount_q05"] - core["pamount_min"]
    ).where((core["pamount_q05"] - core["pamount_min"]).abs().gt(EPS))
    core["trajectory_illiq"] = np.log1p(work["absret"]).groupby(
        [work["date"], work["instrument_id"]], sort=False
    ).sum().to_numpy() / core["amount_sum"].where(core["amount_sum"].gt(0))
    core["twap_position"] = (core["close_mean"] - core["low_min"]) / (
        core["high_max"] - core["low_min"]
    ).where((core["high_max"] - core["low_min"]).gt(EPS))
    ...
    for index, (low, high) in enumerate(
        ...
        core[f"price_q{index}_volume_share"] = volume.to_numpy() / core["volume_sum"].where(
    ...
    for threshold in (5, 10, 15, 20):
        ...
        for label, weight in (
            ...
            denominator = weight.groupby([work["date"], work["instrument_id"]], sort=False).sum(
                min_count=1
            )
            core[label] = (
    ...
    extras = [
        ...
        _corr_columns(rank_work, "ret_rank", "volume_rank", "rank_corr_ret_volume"),
    ...
    output = output.rename(
        columns={column: f"{prefix}_{column}" for column in output.columns if column not in keys}
    )
    return output
...
def _one_minute_extras(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    daily["pvol_diff_cv"] = daily["pvol_diff_std"] / daily["pvol_mean"].where(
        daily["pvol_mean"].abs().gt(EPS)
    )
    ...
    daily["open5_volume_share"] = work["volume"].where(open_mask, 0.0).groupby(
        [work["date"], work["instrument_id"]], sort=False
    ).sum().to_numpy() / daily["total_volume"].where(daily["total_volume"].gt(0))
    ...
    for sigma in (0, 1, 2):
        ...
        for gap in (0, 1, 5):
            ...
            daily[f"peak_{sigma}_{gap}"] = (
                selected.groupby([work["date"], work["instrument_id"]], sort=False).sum().to_numpy()
            )
    return daily
    ...
    for frequency in (5, 10, 15, 30, 60):
        ...
        if frequency >= 10:
            ...
            keep = ["date", "instrument_id"] + [
                ...
                if column.startswith(prefix) and column.removeprefix(prefix) in exact
            ]
            part = part[keep].copy()
        output = output.merge(
            part,
...
def _rolling_pooled_cv(base: pd.DataFrame, prefix: str, field: str, window: int = 20) -> pd.Series:
...
def _rolling_corr(
    ...
    for index in base.groupby("instrument", sort=False).groups.values():
        ...
        output.loc[positions] = (
            local_left.rolling(window, min_periods=minimum).corr(local_right).to_numpy()
        )
    return output
...
def build_panel(base: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, str]]:
        ...
        for frequency in (5, 10):
            for weighted in (False, True):
                ...
                raw = (
                    ...
                    if rest
                    else (base[f"m{frequency}_first_ret"] * base[f"m{frequency}_first_volume"])
                    if weighted
    ...
    direct = {
        ...
        "CJ-G071-V01": (
            "rolling_corr_vol_price",
            "corr_21d(daily intraday volatility, daily mean price)",
        ),
        ...
        "CJ-G092-V01": (
            "m1_absret_weighted_close",
            "mean_21d(volatility-weighted mean price/TWAP)",
        ),
        ...
        "CJ-G112-V01": (
            "daily_rank_corr_volume_close",
            "rank corr_21d(daily volume, adjusted close)",
        ),
    ...
    for source_id, (column, formula) in direct.items():
        ...
        if column == "ret" and source_id == "CJ-G038-V01":
            ...
            values = _rolling_corr(base, base["m1_ret_std"], base["m1_close_mean"], 21)
        ...
        if "up2" in column or "down2" in column:
            values = _rolling(base, np.sqrt(base[column].clip(lower=0)), 21, "mean")
        if column.endswith(("_high_close", "_low_close")):
            values = values / mean21("m1_close_mean").where(mean21("m1_close_mean").abs().gt(EPS))
    ...
    add(
        "CJ-G060-V03",
        _rolling(base, base["m1_vwret"], 21, "mean"),
        "interday mean of daily vw reversal",
    )
    ...
    add(
        "CJ-G063-V02",
        _rolling(base, base["m1_corr_volume_close"], 21, "std"),
        "std_21d(daily price-volume corr)",
    )
    prev_close = base["close"].groupby(base["instrument"], sort=False).shift(1)
    add(
        "CJ-G064-V01",
        _rolling(base, np.log(base["open"] / prev_close), 21, "mean"),
        "mean_21d(log(open/previous_close))",
    )
    add(
        "CJ-G064-V02",
        _rolling(base, np.log(base["first_close"] / prev_close), 21, "mean"),
        "mean_21d(log(first_minute_close/previous_close))",
    )
    add(
        "CJ-G064-V03",
        _rolling(base, np.log(base["first_close"] / base["open"]), 21, "mean"),
        "mean_21d(log(first_minute_close/open))",
    )
    add("CJ-G065-V01", mean21("m1_vwret"), "full-session volume-weighted reversal")
    add(
        "CJ-G065-V02",
        _rolling(base, np.log(base["close"] / base["open"]), 21, "mean"),
        "mean_21d(close-vs-open return)",
    )
    add(
        "CJ-G065-V03",
        _rolling(base, np.log(base["close"] / base["first_close"]), 21, "mean"),
        "mean_21d(close-vs-first-minute-close return)",
    )
    ...
    add(
        "CJ-G113-V01",
        numerator / denominator.where(denominator.abs().gt(EPS)),
        "21d volume-weighted close skewness",
    )
...
def main() -> None:
    ...
    if PRIMITIVE_PATH.exists():
        ...
        paths = discover_month_paths(RAW_DIR, YEARS)
        workers = int(os.environ.get("BIGALPHA_FACTOR_WIKI_WORKERS", "1"))
        if workers < 1:
            raise ValueError("BIGALPHA_FACTOR_WIKI_WORKERS must be positive")
        ...
        if workers == 1:
            ...
            with ProcessPoolExecutor(max_workers=workers) as executor:
                for index, part in enumerate(executor.map(build_month, paths), start=1):
                    print(
                        f"[{index:02d}/{len(paths)}] {paths[index - 1].name}",
                        flush=True,
                    )
                    parts.append(part)
        primitives = pd.concat(parts, ignore_index=True)
        mapping = pd.read_csv(MAPPING_PATH, usecols=["instrument_id", "instrument"])
    ...
    missing_submission_columns = sorted(set(required_submission_columns).difference(panel.columns))
    if missing_submission_columns:
        raise RuntimeError(
            f"Changjiang submission components were not generated: {missing_submission_columns}"
        )
    ...
    for source_id in expected:
        ...
        min_daily_distinct = int(daily_distinct.min()) if valid_days else 0
        status = (
            "TECHNICAL_PASS"
            if (
                coverage >= 0.25
        ...
        rows.append(
            {
                ...
                "reason": (
                    ...
                    else ("coverage/finite/40-day/50-daily-distinct contract")
                ),
            }
        )
