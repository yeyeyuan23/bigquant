...
2022、2023 两个验证年全部通过；I 与 T 只使用 2019—2021 开发期，不使用任一
验证年决定成员，也不重复 all36。T 以 `LightGBM(screened15)` 为树基准，同时
检查单候选增量和全池删一条件增量。当前全部获准路由如下：

| 候选 | S | I | T | 进入管线 |
|---|---:|---:|---:|---|
| `PV-014` | 通过 | 通过 | 通过 | 三条全部进入 |
| `PV-003` | 通过 | 未通过 | 通过 | 规则复合、LightGBM |
| `PV-009` | 通过 | 通过 | 未通过 | 规则复合、Elastic Net |
| `FR-002`、`FR-006` | 未通过 | 通过 | 通过 | Elastic Net、LightGBM |
| `FR-011`、`PV-001`、`PV-002` | 未通过 | 未通过 | 通过 | 仅 LightGBM |
| `FR-004`、`FR-005`、`OB-001`、`PV-008`、`PV-011`、`PV-013`、`PV-019` | 未通过 | 通过 | 未通过 | 仅 Elastic Net |
| 其余技术有效且三项未通过候选 | 未通过 | 未通过 | 未通过 | 不进入 |
| `PV-007`、`FR-007`、`PV-012` | 技术淘汰 | 不运行 | 不运行 | 不进入 |

三条管线的 2022/2023 `raw_full Rank IC` 分别为：

| 管线 | 2022 | 2023 | 结论 |
|---|---:|---:|---|
| `self_factor_composite` | 0.05580 | 0.05794 | 当前第一 |
| `joint_elastic_net` | 0.03962 | 0.02786 | 第二 |
| `joint_lightgbm` | 0.03885 | 0.02583 | 第三 |

2022 与 2023 的验证地位相同。排序先比较两个验证年中较低的 Rank IC，再比较
两年均值。三条管线均覆盖每个验证年的 242 个交易日。以上结果未读取 2024，
2024 仅在方案冻结后测试。

Elastic Net 的 I 评价与最终训练均对特征和标签做逐日截面标准化，并保存每个
滚动窗权重；当某个候选在某日没有截面离散度时，该日特征固定为中性值 0，
不会删除整日样本。

LightGBM 的最终 T 池为 `FR-002、FR-006、FR-011、PV-001、PV-002、PV-003、
PV-014`。该组在2019—2021相对树基准的 OOS Rank IC 增量为 `0.004789`，
34个20日窗口中73.5%为正，三个开发年均为正。完整日频 OB-001 有730个开发日，
但 T 单候选增量为 `-0.000102`；条件增量 `0.000266` 只在一个开发年为正，因此
不进入最终树模型。HF-001、HF-002 与 OB-002 只有115个开发代表日，未达到
240日 T 数据门槛，不将其记录为树增量失败。
