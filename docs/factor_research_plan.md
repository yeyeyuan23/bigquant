...
5. 每个候选使用自己的有效日期日历；特征与标签均按日转换为中心化截面百分位
   秩，同一日期内缺失或无截面离散度的特征按中性值 0 处理，标签仍必须有效。
   候选之间不得通过共同 `dropna` 改变彼此的股票日样本。
6. 每个候选比较 OOS Rank IC，并记录候选权重、选择频率和与 screened15 的
   最大相关性。候选级 I 通过后只标记为 `pending_passed`，不得直接进入最终
   Elastic Net。所有公开因子和自研因子已按 2019—2021 冻结方向统一为
...
中心化截面百分位秩预处理和固定浅层 LightGBM 参数完成两条配对评价。所有输入
均已冻结为“值越大越好”，树模型对每个特征固定施加正单调约束：
