...
- `joint_elastic_net` 和 `joint_lightgbm` 只读取 `I=通过` 的候选，并与冻结的
  15 个公开因子联合训练；
- `O` 只标注联合模型中的候选更偏核心信息还是重叠信息，不决定是否进入训练。
...
7. 2022 判断三条管线是否通过，2023 只做确认，不再修改成员、参数或模型；
8. 在通过 2022 且通过 2023 确认的管线中，按 2022 `raw_full Rank IC` 从高到低
   冻结提交顺序；若完全并列，优先简单的 `self_factor_composite`；
9. 冻结后才读取 2024。2024 只报告最终测试结果，不得更换赢家、成员或参数。

`core_candidate` 与 `overlap_aware_candidate` 只是 `I=通过` 后对 `O` 的解释标签；
两者都进入联合模型，不产生主池、挑战池或暂时观察状态。
