...
能使用该窗口开始前的数据；开发期全部已完成的样本外窗口汇总后冻结准入成员，
随后才运行 2022、2023。更换数据快照、标签、预处理或候选实现后，S/I/T 缓存
全部失效，必须从候选池重跑。
...
T 只使用 2019—2021 开发期。所有技术有效候选先进入树模型待筛池，满足至少
240 个有效开发日后，按相同股票日、标签、60 日训练、20 日 OOS、预处理和固定
浅层 LightGBM 参数完成两条配对评价：

1. 单候选增量：
   `LightGBM(screened15 + x) - LightGBM(screened15)`；
2. 全池条件增量：
   `LightGBM(全部树可评候选) - LightGBM(全部树可评候选 - x)`。
...
最终用全部 `T=通过` 候选重新训练一次联合树模型，并再次相对
`LightGBM(screened15)` 执行相同的整体增量门槛。候选通过但最终组未通过时，
整条 LightGBM 管线不得进入冻结提交顺序。分裂次数、gain importance 与 SHAP
只作解释，不作准入条件。
