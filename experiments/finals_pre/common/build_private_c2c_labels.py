"""Build exact-next-market-day C2C labels from private one-minute bars."""
...
OUTPUT = DATA / "private_c2c_labels_20250101_20260828.parquet"
AUDIT = DATA / "private_c2c_labels_20250101_20260828.audit.json"
POOL_PATHS = (
...
def load_daily_returns(parts: list[Path]) -> pd.DataFrame:
    ...
    for index, path in enumerate(parts, start=1):
        frame = (
            ...
            .select("date", "instrument", "pre_close", "close")
            .with_columns(pl.col("date").dt.truncate("1d").alias("trade_date"))
            .group_by("trade_date", "instrument")
            .agg(
                pl.col("pre_close").sort_by("date").first().alias("pre_close"),
    ...
    daily["daily_c2c"] = (close / pre_close.where(pre_close > 0) - 1.0).replace(
        [np.inf, -np.inf], np.nan
    )
    return daily[["trade_date", "instrument", "daily_c2c"]]
...
def main() -> int:
    ...
    ).rename(columns={"daily_c2c": "ret_close_to_close"})
    ...
    audit = {
        ...
        "definition": "exact next market day close / exact next market day pre_close - 1",
