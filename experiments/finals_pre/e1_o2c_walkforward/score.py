...
def score_frame(frame: pd.DataFrame, stress_days: set[pd.Timestamp]) -> dict[str, float | int]:
    ic = daily_ic(frame)
    ...
    return {
        ...
        "stress_ic_ir": (float(stress.mean() / stress.std()) if len(stress) >= 2 else float("nan")),
    }
...
def write_svg(daily: pd.DataFrame, blocks: pd.DataFrame, path: Path) -> None:
    ...
    daily_points = " ".join(
        f"{x(day):.1f},{y(value):.1f}" for day, value in zip(dates, daily["rank_ic"], strict=True)
    )
    block_points = " ".join(
...
def main() -> int:
    factor = normalize_keys(pd.read_parquet(OUT / "model/factor_private_60d_20d.parquet"))
    ...
    daily["block"] = daily["date"].map(factor.drop_duplicates("date").set_index("date")["block"])
