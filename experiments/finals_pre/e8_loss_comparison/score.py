...
from experiments.finals_pre.e8_loss_comparison.train import actual_key_hash
...
def aggregate(output):
    ...
    if cube.shape != (len(ARMS), len(SEEDS), 241):
    ...
    lines = [
        ...
        (
            ...
            "双侧中心化 bootstrap 检验，2 个基线对比作 Holm 校正。"
            "固定三个种子，区间不覆盖全部初始化不确定性。"
        ),
        "",
    ]
    for group, title in (("loss", "SmoothL1、L1 与半平方 L2"),):
    ...
    lines += [
        ...
        "",
        (
            "全部结论仅适用于三轮训练预算与固定 0.05 系数。"
            "差异不显著不代表效果等价。多空 Sharpe 未扣交易成本，仅作因子诊断。"
        ),
        "",
        (
            "相同系数不等于相同有效梯度强度。本轮回答直接替换的效果，"
            "不证明分别调参后的最优性能，也不证明增加辅助项的必要性。"
        ),
        "",
    ]
