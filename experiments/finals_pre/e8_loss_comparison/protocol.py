"""E8: fixed architecture and paired seeds; only the auxiliary loss changes."""
...
ARMS = (
    ...
    Arm("l1", "loss", 3, (3, 15, 60)),
    Arm("l2_half", "loss", 3, (3, 15, 60)),
)
...
def protocol_document():
    return {
        "schema_version": 1,
        "experiment": "E8 auxiliary loss comparison",
        "auxiliary_weight": AUX_WEIGHT,
        "losses": {
            "smooth_l1": "mean SmoothL1(prediction,target,beta=1)",
            "l1": "mean abs(prediction-target)",
            "l2_half": "0.5 * mean (prediction-target)^2",
        },
        "primary_loss": "negative centered Pearson, unchanged from E9",
        "scope": "direct replacement at fixed coefficient; not separately tuned optima",
        "seeds": SEEDS,
        "epochs": EPOCHS,
        "label": LABEL,
        "minutes": 240,
        ...
        "tests": "two-sided centered bootstrap; Holm across 2 baseline contrasts; alpha=.05",
        "arms": [
