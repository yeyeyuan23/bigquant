"""E8 serial nine-run driver with sealed inputs and fresh paired training."""
...
from experiments.finals_pre.e8_loss_comparison.prepare import prepare, verify_seal
...
from experiments.finals_pre.e8_loss_comparison.score import aggregate, score_run, verify_run
        ...
        for seed in SEEDS:
            for arm in ARMS:
                ...
                status.update(
                    ...
                    updated_at=now(),
                    estimated_remaining_hours=(status["total"] - status["completed"])
                    * sum(durations)
                    / len(durations)
                    / 3600,
                )
