...
TARGET=6
cd "$P"
running() { pgrep -fc "evaluate_unifie[d]|evaluate_ablatio[n]" 2>/dev/null || echo 0; }
claimed() { ls "$OUT/claims" 2>/dev/null | grep -vc '^\.' || echo 0; }

echo "[SUPERVISE START] $(date -Is) target=$TARGET"
while [ "$(claimed)" -lt 27 ] || [ "$(running)" -gt 0 ]; do
...
for pass in 1 2; do
  ...
  sleep 90
  while [ "$(claimed)" -lt 27 ] || [ "$(running)" -gt 0 ]; do sleep 60; done
done
...
git add -A
git commit -q -m "E9 结果：种子与标签矩阵 27 个 run (done=$ok fail=$bad)" || echo "[commit] 无改动"
