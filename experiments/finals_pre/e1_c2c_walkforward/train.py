...
from bigalpha2026.alpha_models import MICROSTRUCTURE_CHANNELS, MicrostructureConfig
...
STORE_VIEW = Path("/root/autodl-tmp/e1_c2c_walkforward_combined_store")
PRIVATE_LABELS = Path(
    "/root/bigquant_private_data/private_c2c_labels_20250101_20260828.parquet"
)
OUT = ROOT / "reports/dependencies/finals_pre/e1_c2c_walkforward/model"
...
def build_store_view() -> dict[str, object]:
    ...
    if STORE_VIEW.exists():
        if STORE_VIEW != Path("/root/autodl-tmp/e1_c2c_walkforward_combined_store"):
...
def load_targets() -> tuple[pd.DatetimeIndex, dict[pd.Timestamp, pd.Series]]:
    ...
    if labels.duplicated(["date", "instrument"]).any():
        raise RuntimeError("combined C2C labels contain duplicate keys")
