...
PULL_PID=/tmp/e5_parquet_pull.pid
PULL_LOG=/tmp/e5_parquet_pull.log
...
echo "[wait] E5 Parquet pull $(date --iso-8601=seconds)"
...
/root/autodl-tmp/conda-envs/quant/bin/python \
  "$CODE/e5_audit_parquet_store.py" \
  --store "$STORE" \
  --output "$OUT/parquet_store_audit.json"

echo "[smoke] CUDA loader/model $(date --iso-8601=seconds)"
/root/autodl-tmp/conda-envs/quant/bin/python \
  "$CODE/e5_autodl_smoke.py" \
  --root "$CODE" \
  --store "$STORE" \
  --device cuda \
  >"$OUT/cuda_smoke.json"

echo "[train] E5 formal AutoDL runs $(date --iso-8601=seconds)"
bash "$CODE/run_e5_autodl.sh"
