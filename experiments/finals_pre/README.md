...
| **E12** | 把未充分使用的 bar1m 字段加工成 9 个新增通道（17 → 26） | `e12_full_fields/` | `e12_full_fields/` |
| **E13** | 三个训练截止点的冻结模型年度老化检验（四个 60 日块） | `e13_frozen_decay/` | `e13_frozen_decay/` |
| **E13b** | E10/E11/E12 被测信息增量的年度内老化检验（四个 60 日块） | `e13b_component_age/` | `e13b_component_age/` |
| **E14** | 单 k=15 模型渐进简化（统计通路、单路融合、DeepSets、MLP 头） | `e14_minimal_k15/` | `e14_minimal_k15/` |
...
主消融协议为 expanding 训练、起点 2019-01-02、标签隔离 1 个交易日（代码硬断言）、
epochs=3、lr=4e-4、max_stocks=1200、2024 整年 untouched 预测、RTX 4090D。
五 seed 稳健性组为 {20260801, 20260812, 20260823, 20260904, 20260915}；
预先指定的三 seed 配对实验使用前三个。E13 是明确的跨年度例外：分别冻结预测
2022、2023、2024，以检验模型年龄，不与只看 2024 的消融臂混为同一统计问题。
E13b 不重新训练，而是把 E10–E12 现有的 2024 日度配对增量切成四个 60 日块，
直接检验年度汇总是否掩盖只在首块存在的增量。
E14 复用 E9 的三 seed 单 k=15 基线，并训练四个嵌套简化臂；它仍只评估 2024，
属于结构筛选实验，不能单独证明简化结构跨年份非劣。
