...
for seed in 20260801 20260812 20260823; do
  ...
  "$python_bin" scripts/evaluate_unified_microstructure.py \
    ...
    --max-minutes 240 --max-stocks 1200 --min-train-days 900 \
    --learning-rate 4e-4 --seed "$seed" --label-column ret_next_open_to_close \
    --eval-every-epoch --fast-pack
done
"$python_bin" experiments/finals_pre/e2_o2c_epoch_curve/score.py
