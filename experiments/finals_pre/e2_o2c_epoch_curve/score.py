"""Score every O2C epoch snapshot with the four A components."""
...
def main() -> int:
    ...
    exposure = prepare_full_barra_exposures(
        pd.read_parquet("/root/autodl-tmp/exposure_2024_full.parquet")
    )
    ...
    for seed in SEEDS:
        ...
        for epoch in range(1, 7):
            path = run / (
                f"unified_microstructure_block_00_checkpoint_epoch_{epoch:02d}_oos.parquet"
            )
            ...
            if len(ic) != 241:
                raise RuntimeError(
                    f"expected 241 O2C days, got {len(ic)}: seed={seed}, epoch={epoch}"
                )
