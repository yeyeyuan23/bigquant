"""One fresh arm, using canonical inputs/loss and audited experimental RNG streams."""
...
def checked_update(model, inputs, target, optimizer, scaler, device):
...
def train_arm(prepared, arm_name, seed, output, device_name="cuda"):
    ...
    try:
        ...
        torch.save(
            {"config": asdict(config_for(arm)), "state_dict": model.cpu().state_dict()}, checkpoint
        )
