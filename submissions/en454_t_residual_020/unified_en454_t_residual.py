"""Formal Candidate454 ElasticNet plus frozen residual-T expert for AIStudio."""
...
FROZEN_MANIFEST = {
    "route": "EN454_T_residual_w020",
    "model": "rolling_elastic_net_plus_frozen_residual_temporal",
    "checkpoint_sha256": "f08859257c3b56bbcf2bf226a8982529bfcea708026322cdc953cfa5293f74e8",
    "checkpoint_training_end": "2023-06-30",
    "selection_evidence": "2023 strict OOS EN454 addback: w=0.20 maximized A, raw B model score, and J among tested weights",
    "training": True,
    ...
    "residual_t_weight": RESIDUAL_T_WEIGHT,
    "history_calendar_days": 183,
    "lookback_trading_days": 60,
    "candidate_count": 454,
    "allowed_sources": ["bar1m", "financial", "stock_pool_keys"],
    "evidence_boundary": (
        "EN454 is retrained causally online; residual-T is frozen. The 20% addback "
        "was selected on 2023 strict OOS local proxy evidence, not a platform score."
    ),
}
...
def _load_checkpoint(torch):
    import zlib
    from unified_en454_t_residual_weights import CHECKPOINT
...
def _daily_cross_sectional_zscore(values, np, eps=1e-6):
    ...
    return np.where(observed, centered / scale, 0.0).astype("float32", copy=False)


def _next_open_close_targets(daily_prices, pd, np):
    daily = daily_prices.loc[:, ["date", "instrument", "open", "close"]].copy()
    ...
    next_prices = daily.rename(
        columns={"date": "next_date", "open": "next_open", "close": "next_close"}
    )
    labels = daily.loc[:, list(KEY_COLUMNS)].merge(
        calendar, on="date", how="left", validate="many_to_one"
    ).merge(
        next_prices.loc[:, ["next_date", "instrument", "next_open", "next_close"]],
    ...
    labels["target"] = (
        raw.groupby(labels["date"], sort=False).rank(method="average", pct=True)
        * 2.0
        - 1.0
    )
...
def _fit_elasticnet(panel, targets, start, end, candidate_ids, pd, np):
    ...
    requested = dates[(dates >= start) & (dates <= end)]
    if requested.empty:
    ...
    instrument_positions = {
        instrument: position for position, instrument in enumerate(instruments)
    }
    rows = []
    for offset in range(0, len(requested), PREDICTION_DAYS):
        ...
        prediction_values = _daily_cross_sectional_zscore(values[prediction], np)
        raw_predictions = model.predict(
            prediction_values.reshape(-1, len(candidate_ids))
        ).reshape(len(prediction), len(instruments))
        for row_index, date in enumerate(prediction_dates):
            keys = panel.loc[panel["date"].eq(date), list(KEY_COLUMNS)].copy()
            positions = np.asarray(
                [instrument_positions[value] for value in keys["instrument"]],
                dtype=int,
            )
            keys["factor"] = _rank(raw_predictions[row_index, positions], pd)
            rows.append(keys)
    return pd.concat(rows, ignore_index=True)


def main(datasources, start_date, end_date):
    """Return the causal EN454 signal with a 20% frozen residual-T addback."""
    ...
    feature_start = start - pd.Timedelta(days=MODEL_HISTORY_CALENDAR_DAYS)
    ...
    temporal = pd.concat(temporal_outputs, ignore_index=True)
    targets = _next_open_close_targets(daily_prices, pd, np)
    elasticnet = _fit_elasticnet(
        panel, targets, start, end, candidate_ids, pd, np
    )
    output = elasticnet.rename(columns={"factor": "elasticnet"}).merge(
        temporal,
        on=list(KEY_COLUMNS),
        how="inner",
        validate="one_to_one",
    ...
    output["factor"] = (
        (1.0 - RESIDUAL_T_WEIGHT) * output["elasticnet"]
        + RESIDUAL_T_WEIGHT * output["residual_t"]
    )
    output["factor"] = (
        output.groupby("date", sort=False)["factor"].rank(
    ...
    if output.empty or output.duplicated(list(KEY_COLUMNS)).any():
        raise RuntimeError("EN454 + residual-T output is empty or duplicated")
    if not np.isfinite(output["factor"].to_numpy()).all():
        raise RuntimeError("EN454 + residual-T output contains non-finite values")
    if output.groupby("date")["factor"].nunique().le(1).any():
        raise RuntimeError("EN454 + residual-T output contains a constant day")
    return output
...
def build_candidate454(
    ...
    *,
    ...
    output = wide.loc[wide["date"].between(lower, end)].reset_index(drop=True)
    if not return_daily_prices:
        return output
    prices = daily.loc[
        daily["date"].between(lower, end),
