"""Frozen Candidate454 MLP X factor for AIStudio inference."""
...
def _masked_cross_sectional_zscore(
    values, observed_mask, stock_mask, torch, eps=1e-6
):
    valid = (
        observed_mask.bool()
        & stock_mask[:, :, None, None].bool()
...
def _candidate_mlp_network(torch):
    class CandidateMLPNetwork(torch.nn.Module):
        def __init__(
            self,
            input_dim=454,
            hidden_dims=(512, 256),
            ...
            hidden = tuple(int(value) for value in hidden_dims)
            widths = (self.input_dim * 2, *hidden, 1)
            layers = []
            for in_features, out_features in pairwise(widths[:-1]):
                layers.extend(
                    (
    ...
    return CandidateMLPNetwork


def _load_model(torch):
    from unified_x_weights import (
        CHECKPOINT_B85,
        CHECKPOINT_BYTES,
        CHECKPOINT_SHA256,
    )

    checkpoint_bytes = base64.b85decode(b"".join(CHECKPOINT_B85))
    if len(checkpoint_bytes) != CHECKPOINT_BYTES:
        raise RuntimeError("embedded X checkpoint byte count mismatch")
    if hashlib.sha256(checkpoint_bytes).hexdigest() != CHECKPOINT_SHA256:
        raise RuntimeError("embedded X checkpoint SHA-256 mismatch")
    payload = torch.load(
        io.BytesIO(checkpoint_bytes), map_location="cpu", weights_only=True
    )
    config = dict(payload["config"])
    if int(config.get("input_dim", -1)) != 454:
        raise RuntimeError(f"unexpected frozen X input_dim: {config.get('input_dim')}")
    spec = payload.get("candidate454_spec")
    if not isinstance(spec, dict):
        raise RuntimeError("frozen X checkpoint is missing Candidate454 metadata")
    required = {"candidate_ids", "component_specs", "gtja_specs"}
    if not required.issubset(spec):
        raise RuntimeError("frozen X checkpoint has incomplete Candidate454 metadata")
    CandidateMLPNetwork = _candidate_mlp_network(torch)
    model = CandidateMLPNetwork(**config)
...
def main(datasources, start_date, end_date):
    """Return the required ``date, instrument, factor`` prediction frame."""

    import numpy as np
    import pandas as pd
    import torch

    model, candidate_spec = _load_model(torch)
    candidate_ids = tuple(candidate_spec["candidate_ids"])
    panel = build_candidate454(
        datasources, start_date, end_date, candidate_spec=candidate_spec
    )
    if panel.empty:
        raise RuntimeError("Candidate454 runtime produced no prediction rows")
    ...
    with torch.inference_mode():
        for _, block in panel.groupby("date", sort=True):
            values_np = block.loc[:, list(candidate_ids)].to_numpy(
                dtype="float32", copy=True
            )
            values = torch.from_numpy(values_np).unsqueeze(0).to(device)
            observed = torch.isfinite(values)
            stock_mask = torch.ones(
                (1, len(block)), dtype=torch.bool, device=device
            )
            if device.type == "cuda":
                with torch.amp.autocast("cuda", dtype=torch.float16):
                    prediction = model(values, observed, stock_mask).squeeze(0)
            else:
                prediction = model(values, observed, stock_mask).squeeze(0)
            raw = prediction.float().cpu().numpy()
            factor = pd.Series(raw).rank(method="average", pct=True) * 2.0 - 1.0
            part = block.loc[:, list(KEY_COLUMNS)].copy()
            part["factor"] = factor.to_numpy(dtype="float64")
            rows.append(part)
    ...
    if output.empty:
        raise RuntimeError("frozen X produced no valid prediction rows")
    if output.duplicated(list(KEY_COLUMNS)).any():
        raise RuntimeError("frozen X produced duplicate output keys")
    if not np.isfinite(output["factor"].to_numpy(dtype="float64")).all():
        raise RuntimeError("frozen X produced non-finite factor values")
    if output.groupby("date", sort=False)["factor"].nunique().le(1).any():
        raise RuntimeError("frozen X produced a constant daily cross-section")
...
def build_candidate454(
    datasources, start_date, end_date, *, candidate_spec
):
    import dai
    import numpy as np
    import pandas as pd
    ...
    return wide.loc[wide["date"].between(start, end)].reset_index(drop=True)
