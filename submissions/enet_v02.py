"""Platform-safe Elastic Net factor from the refreshed I pool.
...
positive coefficients, and only labels fully observed before start_date.
"""
...
def main(datasources, start_date, end_date):
    ...
    trailing_high = (
        close_price.groupby(panel["instrument"], sort=False)
    ...
    panel["PV-008"] = ranked(
        close_price / trailing_high.where(trailing_high > 0)
    )

    # PV-013: twelve-to-one-month momentum.
    grouped_close = close_price.groupby(panel["instrument"], sort=False)
    old_price = grouped_close.shift(252)
    recent_price = grouped_close.shift(21)
    panel["PV-013"] = ranked(recent_price / old_price.where(old_price > 0) - 1.0)

    # PV-011: twelve-to-six-month intermediate momentum.
    old_price = grouped_close.shift(252)
    recent_price = grouped_close.shift(126)
    panel["PV-011"] = ranked(recent_price / old_price.where(old_price > 0) - 1.0)
    ...
    # PV-019: CAPM-residual momentum, skipping the latest 21 trading days.
    mean_ri_252 = (
    ...
    mean_rm_252 = (
        panel.groupby("instrument", sort=False)["market_return"]
        .rolling(252, min_periods=200)
        .mean()
        .reset_index(level=0, drop=True)
        .sort_index()
    ...
    cov_252 = (
        panel.groupby("instrument", sort=False)["ri_rm"]
        .rolling(252, min_periods=200)
        .mean()
        .reset_index(level=0, drop=True)
        .sort_index()
        - mean_ri_252 * mean_rm_252
    )
    var_rm_252 = (
        panel.groupby("instrument", sort=False)["rm2"]
        .rolling(252, min_periods=200)
        .mean()
        .reset_index(level=0, drop=True)
        .sort_index()
    ...
    beta_252 = cov_252 / var_rm_252.where(var_rm_252 > 0)
    residual = panel["stock_return"] - (
        mean_ri_252 + beta_252 * (panel["market_return"] - mean_rm_252)
    )
    lagged_residual = residual.groupby(panel["instrument"], sort=False).shift(21)
    residual_mean = (
        lagged_residual.groupby(panel["instrument"], sort=False)
    ...
    panel["PV-019"] = ranked(residual_mean / residual_std.where(residual_std > 0))
    ...
    # FR-006: PIT year-over-year change in TTM net profit.
    fr006_parts = []
    for _, block in ttm.groupby("instrument", sort=False):
        history = {}
        values = []
        ...
        enriched["FR-006"] = values
        fr006_parts.append(enriched)
    fr006_events = (
        pd.concat(fr006_parts, ignore_index=True)
        .sort_values(["instrument", "disclosure_date"])
        .drop_duplicates(["instrument", "disclosure_date"], keep="last")
    )
    fr006_state = group_asof(
        panel[["date", "instrument"]],
        fr006_events,
        "disclosure_date",
        ["FR-006"],
    )
    panel["FR-006"] = ranked(fr006_state["FR-006"])
    ...
    # A training exposure on the trading day immediately before the first
    # prediction would use the first prediction day's return as its label.
    # Exclude it so every training label is fully observed before start_date.
    train_end_position = first_test_position - 1
    train_start_position = train_end_position - 60
    if train_start_position < 0:
        raise ValueError("not enough pre-start history for Elastic Net training")
    ...
    if train.empty or test.empty:
        raise ValueError("empty Elastic Net train or prediction sample")
    model = ElasticNet(
        alpha=0.001,
        l1_ratio=0.5,
        fit_intercept=True,
