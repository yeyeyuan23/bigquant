"""Frozen Candidate454 LightGBM X-tree factor for AIStudio inference."""
...
FROZEN_MANIFEST = {
    "route": "X_tree",
    "model": "LightGBM",
    "checkpoint": "unified_lightgbm_2024_h2_checkpoint.txt",
    "training_end": "2024-06-30",
    "feature_bundle": "Candidate454",
    "training": False,
    "allowed_sources": ["bar1m", "financial", "stock_pool_keys"],
    "evidence_boundary": "Frozen 2024-H2 checkpoint; platform score is not known until submission.",
}


def _load_model():
    import lightgbm as lgb
    from unified_x_tree_weights import (
        CANDIDATE_SPEC_B85_ZLIB,
    ...
    model_bytes = zlib.decompress(base64.b85decode(MODEL_B85_ZLIB))
    if len(model_bytes) != MODEL_BYTES:
        raise RuntimeError("embedded X-tree model byte count mismatch")
    if hashlib.sha256(model_bytes).hexdigest() != MODEL_SHA256:
        raise RuntimeError("embedded X-tree model SHA-256 mismatch")
    spec_bytes = zlib.decompress(base64.b85decode(CANDIDATE_SPEC_B85_ZLIB))
    if len(spec_bytes) != CANDIDATE_SPEC_BYTES:
        raise RuntimeError("embedded Candidate454 spec byte count mismatch")
    if hashlib.sha256(spec_bytes).hexdigest() != CANDIDATE_SPEC_SHA256:
        raise RuntimeError("embedded Candidate454 spec SHA-256 mismatch")
    candidate_spec = json.loads(spec_bytes.decode("utf-8"))
    candidate_ids = tuple(candidate_spec.get("candidate_ids", ()))
    if len(candidate_ids) != 454 or len(set(candidate_ids)) != 454:
        raise RuntimeError("embedded Candidate454 order is invalid")
    model = lgb.Booster(model_str=model_bytes.decode("utf-8"))
    expected_names = [f"candidate__{candidate_id}" for candidate_id in candidate_ids]
    if model.num_feature() != 454 or model.feature_name() != expected_names:
        raise RuntimeError("X-tree checkpoint/Candidate454 feature order mismatch")
    return model, candidate_spec, candidate_ids


def _finite_rank(values, pd):
    series = pd.to_numeric(pd.Series(values), errors="coerce")
    finite = series.notna() & series.abs().lt(float("inf"))
    if int(finite.sum()) < 2:
        raise RuntimeError("X-tree produced fewer than two finite daily predictions")
    factor = pd.Series(0.0, index=series.index, dtype="float64")
    factor.loc[finite] = (
        series.loc[finite].rank(method="average", pct=True).to_numpy(dtype="float64")
        * 2.0
        - 1.0
    )
    return factor.to_numpy(dtype="float64")
...
def main(datasources, start_date, end_date):
    ...
    model, candidate_spec, candidate_ids = _load_model()
    ...
    if panel.empty or panel.duplicated(list(KEY_COLUMNS)).any():
        raise RuntimeError("Candidate454 runtime is empty or duplicated")
    missing = sorted(set(candidate_ids).difference(panel.columns))
    unexpected = sorted(set(panel.columns).difference({*KEY_COLUMNS, *candidate_ids}))
    if missing or unexpected:
        raise RuntimeError(
            f"Candidate454 schema mismatch: missing={missing}, unexpected={unexpected}"
        )
    ...
    for day, block in panel.groupby("date", sort=True):
        ...
        part["factor"] = _finite_rank(raw, pd)
        parts.append(part)
    ...
    if output.empty or output.duplicated(list(KEY_COLUMNS)).any():
        raise RuntimeError("X-tree output is empty or duplicated")
    if not np.isfinite(output["factor"].to_numpy(dtype="float64")).all():
        raise RuntimeError("X-tree output contains non-finite values")
    if output.groupby("date", sort=False)["factor"].nunique().le(1).any():
        raise RuntimeError("X-tree output contains a constant daily cross-section")
    return output
