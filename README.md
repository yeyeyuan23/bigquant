...
规则组合和机器学习基线已经完整比较：单因子、等权秩、75/25、训练期 IC 加权、Elastic Net 和浅层 HistGradientBoosting 均走完同一评价接口。2022 选择期中等权秩组合 Rank IC 为 `0.00851`，高于 `FR-002` 的 `0.00610`；2023 只作确认，等权组合 Rank IC 为 `0.01710`。Elastic Net 的 2022 Rank IC 为 `-0.00404`；树模型为 `0.00630`，但扩展窗口开发期为负、2023 仅 `0.00101`，两者均未替换等权组合。
